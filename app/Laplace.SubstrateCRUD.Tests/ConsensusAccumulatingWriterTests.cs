...
public class ConsensusAccumulatingWriterTests
{
    ...
    public async Task Accumulation_FoldsSignedMultisetIntoConsensus()
    {
        ...
        var rows = new[]
        {
            ...
            Obs(H(2003), s2, kind, o1, src, score: 100_000_000, ctx: c1),
        ...
        var unary = await ConsensusRowAsync(s1, kind, null);
    ...
    public async Task Accumulation_PersistsProvenanceOnlyEvidence()
    {
        ...
        var r = await accumulator.ApplyManyAsync(new[]
            { Change(src, "acc-prov",
                Obs(H(230), subj, kind, obj, src, 900_000_000),
                Obs(H(231), subj, kind, null, src, 100_000_000)) });
        ...
        Assert.NotNull(await ConsensusRowAsync(subj, kind, obj));
    }
