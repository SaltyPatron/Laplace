...
- **Folder restructure required.** `extension/` becomes `extension/{laplace_geom,laplace_substrate}/`, each with its own `CMakeLists.txt` (per [ADR 0032](0032-unified-cmake-build-pipeline.md); PGXS retired), `.control`, `.sql.in` modules (per [ADR 0034](0034-modular-sql-via-cpp-preprocessor.md)), `src/`, `tests/` subdirs.
- **CI build/install steps double up** — `cmake --install build` covers both extensions; per [ADR 0045](0045-laplace-admin-superuser-supersedes-laplace-priv-wrapper.md) the install is staged into the `laplace-runner`-owned prefix so no sudo is required.
