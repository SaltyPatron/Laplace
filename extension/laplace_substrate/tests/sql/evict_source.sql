...
BEGIN
    ...
    VALUES
        ('00000000-0000-0000-0000-000000000508', 'EvictTestAnalysis', src_a, 2, 'ok');
    INSERT INTO laplace.ingest_file_journal
        (run_id, file_label, source_name, file_id, status)
    VALUES
        ('00000000-0000-0000-0000-000000000508', 'owned.file', 'EvictTestAnalysis', f1, 'ok'),
        ('00000000-0000-0000-0000-000000000508', 'legacy.file', 'EvictTestAnalysis', f2, 'ok');
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'FAIL: source-owned replay claim survived source eviction';
    END IF;
    IF (SELECT count(*) FROM laplace.ingest_file_journal
        WHERE run_id = '00000000-0000-0000-0000-000000000508') <> 2 THEN
        RAISE EXCEPTION 'FAIL: eviction destroyed the per-file audit journal';
    END IF;
    ...
    expect := laplace.laplace_glicko2_accumulate_period(
        ...
        ARRAY[consensus.glicko2_neutral_mu()]::bigint[], ARRAY[phi_w]::bigint[],
        ARRAY[3]::bigint[], ARRAY[3 * s_conf]::bigint[], consensus.glicko2_tau());
    SELECT * INTO row_c FROM laplace.consensus
    ...
    -- (VI): one canonical aggregate is one rating period. Both surviving rows
    -- contribute before the state transition; observation timestamps retain
    -- provenance and do not invent unrecoverable original batch boundaries.
    expect := laplace.laplace_glicko2_accumulate_period(
        consensus.glicko2_neutral_mu(), consensus.glicko2_initial_rd(), consensus.glicko2_initial_volatility(),
        ARRAY[consensus.glicko2_neutral_mu(), consensus.glicko2_neutral_mu()]::bigint[],
        ARRAY[phi_w, phi_w]::bigint[],
        ARRAY[1, 2]::bigint[], ARRAY[win, s_conf]::bigint[], consensus.glicko2_tau());
    SELECT * INTO row_c FROM laplace.consensus
    ...
    IF row_c.rating <> expect.rating OR row_c.rd <> expect.rd
       OR row_c.volatility <> expect.volatility THEN
        RAISE EXCEPTION 'FAIL: interleaved refold diverges from one grouped survivor period';
    END IF;
...
DECLARE
    ...
    subj  bytea := public.laplace_hash128_blake3('test/evict/subject');
BEGIN
    DELETE FROM laplace.ingest_file_journal
    ...
    DELETE FROM laplace.entities WHERE id IN (
        ...
        public.laplace_hash128_blake3('test/evict/file/owned'),
        public.laplace_hash128_blake3('test/evict/file/legacy'));
    DROP TABLE IF EXISTS _ev_snap_v1;
    DROP TABLE IF EXISTS _ev_snap_evicted;
    RAISE NOTICE '- evict fixture cleaned up';
END $$;

-- DEFAULT is hash-subpartitioned into eight leaves. Nine dynamic relation types
-- guarantee at least one leaf contains more than one type; relation discovery
-- must enumerate every distinct type below the DEFAULT ancestor, not LIMIT 1 per
-- hash leaf and require repeated operator invocations to finish one source.
DO $$
DECLARE
    type_t bytea := public.laplace_hash128_blake3('Type');
    src    bytea := laplace.source_id('EvictDefaultPartitionTest');
    subj   bytea := public.laplace_hash128_blake3('test/evict/default/subject');
    rel    bytea;
    obj    bytea;
    evid   bytea;
BEGIN
    ...
    FOR i IN 1..9 LOOP
        rel := public.laplace_hash128_blake3(
            convert_to('test/evict/default/relation/' || i, 'UTF8'));
        obj := public.laplace_hash128_blake3(
        ...
        INSERT INTO laplace.attestations
            (id, subject_id, type_id, object_id, source_id, context_id,
             ...
             sum_score_fp1e9, opponent_rd_fp1e9)
        VALUES
            (evid, subj, rel, obj, src, NULL,
             2, '2026-04-01 00:00:00+00', 1, 1000000000, 30000000000);
    END LOOP;
    RAISE NOTICE '- default-subpartition fixture deposited: 9 dynamic relation types';
END $$;

CALL ops.evict_source(laplace.source_id('EvictDefaultPartitionTest'), NULL, NULL,
                  p_batch => 2, p_drain => false);
...
DECLARE
    ...
    n    bigint;
BEGIN
    SELECT count(*) INTO n FROM laplace.attestations WHERE source_id = src;
    IF n <> 0 THEN
        RAISE EXCEPTION 'FAIL: DEFAULT-subpartition discovery left % dynamic relation row(s)', n;
    END IF;

    DELETE FROM laplace.highway_mask_dirty WHERE id = subj;
    FOR i IN 1..9 LOOP
        rel := public.laplace_hash128_blake3(
            convert_to('test/evict/default/relation/' || i, 'UTF8'));
        obj := public.laplace_hash128_blake3(
            convert_to('test/evict/default/object/' || i, 'UTF8'));
        DELETE FROM laplace.highway_mask_dirty WHERE id = obj;
        DELETE FROM laplace.entities WHERE id IN (rel, obj);
    END LOOP;
    DELETE FROM laplace.entities WHERE id IN (src, subj);
    RAISE NOTICE '- default-subpartition discovery evicted all dynamic relations in one call';
END $$;
