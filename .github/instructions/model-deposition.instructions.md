...
- In the current `ModelTableETL` path, tensor bytes feed four token→token relation types: `SIMILAR_TO` (embedding cosine), `ATTENDS` (Q·K bilinear), `OV_RELATES` (V·O projection vs unembedding), and `COMPLETES_TO` (FFN contraction vs unembedding). Intermediate spaces (neuron, attn_dim) are contracted away inside native kernels and never surface as substrate entities. `ModelDecomposer.InitializeAsync` seeds tensor-role kinds for the export contract.
