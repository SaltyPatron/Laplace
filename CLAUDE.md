...
- **Data**: corpora `D:\Data\Ingest` (books: `D:\Data\Ingest\test-data\text`; Unicode UCD: `D:\Data\Ingest\UCD\Public\UCD\latest` — the old `Unicode\Public\17.0.0` tree is quarantined as `Unicode.BAD-DONOTUSE`, do not use it); models `D:\Models\hub`; junctions `D:\vault\Data`→Ingest, `D:\vault\models`→hub.
...
- Any process running TextDecomposer / the content witness must call `CodepointPerfcache.LoadDefault()` first (Engine.Core). As of 2026-06-11 the witness FAILS LOUD when the T0 perfcache is unloaded — `IntentStage.TryAddContentWitness` throws `InvalidOperationException` (native rc -3); it used to swallow it into a silent no-op. Separately, the content witness uses a **process-global record-once emit bank** (`g_emit_bank` in `content_witness_batch.c`): a root already banked this run is referenced, NOT re-emitted. Any test/ingest must `IntentStage.ResetContentBank()` at its start, and parallel tests sharing the bank must be serialized — a sibling banking a root mid-run makes another run's attestations reference rows it never wrote (the Unicode-seed 1286-ghost-reference bug, 2026-06-14).
