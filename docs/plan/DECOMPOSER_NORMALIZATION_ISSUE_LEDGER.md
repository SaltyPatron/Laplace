...
| DN-08A | Traversal weights and work bounds come from substrate state or callers | Delivered for A*, edge strength, walk/steer, Foundry planes, and prompt/walk ranking; broader scoring audit open | #1236 derives strength from Glicko-2 expected score, turns path likelihood into additive negative-log cost, removes the engine's 256-neighbor truncation and implicit depth defaults, and uses geometry only as an equal-cost ordering hint. #1237 removes the second post-fold confidence system (`exp(-rd/initial_rd)` and witness half-max 4) so native and SQL paths consume rating/RD once. #1238 replaces walk's additive numeric bonuses with an explicit belief/topic/ordinal/geometry tuple and removes prompt's seed-sized `10^13` half-max plus the cross-unit `specificity * denote_mu` product. | [#588](https://github.com/SaltyPatron/Laplace/issues/588), [#1135](https://github.com/SaltyPatron/Laplace/issues/1135): remove remaining fitted scoring constants and classify every public work bound. |
