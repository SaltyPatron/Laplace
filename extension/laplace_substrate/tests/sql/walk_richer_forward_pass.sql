...
-- fixture), accelerator-mask fail-open parity, and astar_path_raw's opt-in
-- geometry heuristic default-safety. Fixture ids are hand-picked distinct
-- bytes, same convention as entities_exist_bitmap.sql -- not real
...
-- NULL accelerator mask: canonical result remains visible.
SELECT count(*) = 1 AS null_mask_confirmed_present
FROM consensus.walk_branches(
    (SELECT id FROM walk_c_fixtures WHERE name = 'subject'),
    laplace.relation_type_id('COMPLETES_TO'), 1, 10, NULL, NULL);

-- A matching cached mask may accelerate a future implementation, but cannot be
-- semantic authority. It therefore returns the same canonical result today.
...
-- A stale/partial/disjoint cached mask must also return the same answer. This is
-- the accelerator-only law from #529: a mask miss may change the physical plan,
-- never the truth visible to the caller.
SELECT count(*) = 1 AS disjoint_mask_cannot_hide_confirmed
FROM consensus.walk_branches(
    (SELECT id FROM walk_c_fixtures WHERE name = 'subject'),
    laplace.relation_type_id('COMPLETES_TO'), 1, 10,
