...
export function GlomeCanvas({
  ...
  if (nodes.length === 0) {
    return <div className="glome-empty">No S³ coordinates to render.</div>;
  }

  return (
    <div className="glome-canvas">
      {nodes.length > MAX_NODES ? (
        <p className="muted glome-cap">Showing {MAX_NODES} of {nodes.length} nodes</p>
      ) : null}
      ...
      <p className="glome-note">Structural identity on S³ — not semantic embedding.</p>
    </div>
  );
}
