...
def write_postgresql_fixture(prefix: Path, version: str = "18.6") -> None:
    """Actual executable/config/header files; no PostgreSQL server is started."""
    directories = {
        "includedir": prefix / "include",
        "includedir-server": prefix / "include/server",
        "libdir": prefix / "lib", "pkglibdir": prefix / "lib/pgsql",
        "sharedir": prefix / "share", "bindir": prefix / "bin",
    }
    for path in directories.values():
        path.mkdir(parents=True, exist_ok=True)
    for name, label in (("postgres", "postgres (PostgreSQL)"), ("pg_config", "PostgreSQL")):
        replies = {"--version": label + " " + version}
        if name == "pg_config":
            replies.update({"--" + key: str(value) for key, value in directories.items()})
        executable = prefix / "bin" / name
        executable.write_text("#!" + sys.executable + "\nimport sys\n"
                              "replies = " + repr(replies) + "\n"
                              "if len(sys.argv) != 2 or sys.argv[1] not in replies: raise SystemExit(2)\n"
                              "print(replies[sys.argv[1]])\n")
        executable.chmod(0o755)
    major, minor = (int(part) for part in version.split("."))
    (directories["includedir-server"] / "pg_config.h").write_text(
        '#define PG_VERSION "' + version + '"\n#define PG_VERSION_NUM '
        + str(major * 10000 + minor) + "\n")
    (directories["includedir-server"] / "postgres.h").write_text("/* fixture server header */\n")
    (directories["includedir"] / "libpq-fe.h").write_text("/* fixture public header */\n")
...
class PostgreSQLReleaseTests(unittest.TestCase):
    ...
    def test_build_inputs_bind_actual_headers_tools_and_ignore_timestamp_churn(self):
        ...
        before = owner.build_inputs(self.prefix, self.selected)
        self.assertFalse(before["running_server_checked"])
        self.assertEqual(str(self.prefix / "include/server"),
                         before["configuration_paths"]["includedir-server"])
        self.assertEqual(2, before["headers"]["includedir-server"]["files"])
        for path in self.prefix.rglob("*"):
            os.utime(path, None)
        self.assertEqual(before, owner.build_inputs(self.prefix, self.selected))
        for path in (self.prefix / "bin/postgres", self.prefix / "bin/pg_config",
                     self.prefix / "include/server/postgres.h", self.prefix / "include/libpq-fe.h"):
            with self.subTest(path=path):
                original = path.read_bytes()
                path.write_bytes(original + b"\n# changed fixture input\n" if path.parent.name == "bin"
                                 else original + b"/* changed header */\n")
                self.assertNotEqual(before, owner.build_inputs(self.prefix, self.selected))
    ...
    def test_old_tools_or_headers_cannot_emit_a_successful_native_fingerprint(self):
        for kind in ("postgres", "pg_config", "headers", "missing"):
            with self.subTest(kind=kind):
                write_postgresql_fixture(self.prefix)
                if kind == "headers":
                    path = self.prefix / "include/server/pg_config.h"
                    path.write_text('#define PG_VERSION "18.3"\n#define PG_VERSION_NUM 180003\n')
                elif kind == "missing":
    ...
    def test_header_version_number_and_complete_header_contents_are_required(self):
        ...
        for text in (original.replace("180006", "180003"), original + original, ""):
            with self.subTest(text=text):
                path.write_text(text)
                with self.assertRaisesRegex(ValueError, "server headers"):
                    owner.build_inputs(self.prefix, self.selected)
        path.write_text(original)
        before = owner.build_inputs(self.prefix, self.selected)
        extra = self.prefix / "include/server/added.h"
        extra.write_text("/* newly installed transitive header */\n")
        self.assertNotEqual(before, owner.build_inputs(self.prefix, self.selected))
        extra.unlink()
        self.assertEqual(before, owner.build_inputs(self.prefix, self.selected))

    def test_selected_include_paths_are_observed_and_invalid_paths_refuse(self):
        write_postgresql_fixture(self.prefix)
        config = self.prefix / "bin/pg_config"
        original = config.read_text()
        selected = self.prefix / "include/server"
        alternate = self.root / "custom-server-headers"
        shutil.copytree(selected, alternate)
        before = owner.build_inputs(self.prefix, self.selected)
        config.write_text(original.replace(str(selected), str(alternate)))
        observed = owner.build_inputs(self.prefix, self.selected)
        self.assertEqual(str(alternate), observed["configuration_paths"]["includedir-server"])
        self.assertNotEqual(before, observed)
        for wrong in ("relative/include", str(self.root / "missing-headers")):
            config.write_text(original.replace(str(selected), wrong))
            with self.assertRaisesRegex(ValueError, "existing absolute directory"):
                owner.build_inputs(self.prefix, self.selected)

    def test_linked_header_subtree_tracks_content_and_refuses_cycles(self):
        write_postgresql_fixture(self.prefix)
        elsewhere = self.root / "shared-headers"
        elsewhere.mkdir()
        header = elsewhere / "shared.h"
        header.write_text("/* first */\n")
        link = self.prefix / "include/server/shared"
        link.symlink_to(elsewhere, target_is_directory=True)
        before = owner.build_inputs(self.prefix, self.selected)
        header.write_text("/* other */\n")
        self.assertNotEqual(before, owner.build_inputs(self.prefix, self.selected))
