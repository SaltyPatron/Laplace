...
OUT_QUALIFIER_FRAG = ROOT / "extension/laplace_substrate/sql/generated/qualifier_bits.sql.in"
...
def emit_qualifier_law(path: Path, relation_names: set[str], relations: list[dict] | None = None) -> None:
    ...
    # SQL readers test a claim's qualifier by name: laplace.qualifier_bit is IMMUTABLE over
    # literals, so a constant (family, value) folds to its bit at plan time, and an
    # undeclared one fails closed rather than reading as "not qualified".
    whens = "\n".join(f"        WHEN '{f}/{v}' THEN {b}" for f, v, b in rows)
    admitted = emit_admitted_evidence_sql(relations or [])
    sql = f"""-- GENERATED by scripts/codegen-attestation-law.py - do not edit.
-- Claim qualifiers (engine/manifest/qualifiers.toml): family/value -> bit of an
-- attestation's qualifier_mask (32 bytes, bit b = get_bit(mask, b)).
CREATE OR REPLACE FUNCTION laplace.qualifier_bit(p_family text, p_value text)
    RETURNS integer
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $q$
DECLARE
    v_bit integer := CASE p_family || '/' || p_value
{whens}
    END;
BEGIN
    IF v_bit IS NULL THEN
        RAISE EXCEPTION 'undeclared claim qualifier %/%', p_family, p_value
            USING HINT = 'engine/manifest/qualifiers.toml';
    END IF;
    RETURN v_bit;
END
$q$;

-- Whether a claim states that qualifier; a NULL mask states none.
CREATE OR REPLACE FUNCTION laplace.has_qualifier(p_mask bytea, p_family text, p_value text)
    RETURNS boolean
    LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $q$
    SELECT p_mask IS NOT NULL
       AND get_bit(p_mask, laplace.qualifier_bit(p_family, p_value)) = 1
$q$;
{admitted}"""
    OUT_QUALIFIER_FRAG.parent.mkdir(parents=True, exist_ok=True)
    _write_text_if_changed(OUT_QUALIFIER_FRAG, sql)


def emit_admitted_evidence_sql(relations: list[dict]) -> str:
    """A retired relation keeps its type id for reading admitted evidence: its rows mean
    the successor element, read in the other direction when retired_flip, carrying the
    retired_qualifier. Readers select an element's rows, old and new, and test a claim's
    qualifier through these, so a retirement never hides evidence already admitted."""
    def tid(name):
        return f"laplace.relation_type_id('{name}')"
    straight, flipped, qualified = {}, {}, {}
    for r in relations:
        successor = r.get("retired")
        if not successor or successor == TRAJECTORY_FACT:
            continue
        (flipped if r.get("retired_flip") else straight).setdefault(successor, []).append(r["canonical"])
        if r.get("retired_qualifier"):
            qualified.setdefault(r["retired_qualifier"], []).append(r["canonical"])
    live = sorted(r["canonical"] for r in relations if not r.get("retired"))
    def case(table, include_self):
        arms = []
        for element in live:
            names = ([element] if include_self else []) + sorted(table.get(element, []))
            if names and (not include_self or table.get(element)):
                arms.append(f"        WHEN '{element}' THEN ARRAY[{', '.join(tid(n) for n in names)}]")
        return "\n".join(arms)
    q_arms = "\n".join(
        f"        WHEN '{q}' THEN ARRAY[{', '.join(tid(n) for n in sorted(names))}]"
        for q, names in sorted(qualified.items()))
    return f"""
-- The type ids whose rows state an element in its own direction: the element and every
-- relation retired into it unflipped. IMMUTABLE over literals, so it folds and prunes.
CREATE OR REPLACE FUNCTION laplace.element_type_ids(p_element text)
    RETURNS bytea[]
    LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $q$
    SELECT CASE p_element
{case(straight, True)}
        ELSE ARRAY[laplace.relation_type_id(p_element)]
    END
$q$;

-- The type ids of relations retired into an element with retired_flip: their admitted
-- rows state the element with subject and object exchanged.
CREATE OR REPLACE FUNCTION laplace.element_flipped_type_ids(p_element text)
    RETURNS bytea[]
    LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $q$
    SELECT CASE p_element
{case(flipped, False)}
...
-- Whether a claim states a qualifier: its mask carries the bit, or its type is a relation
-- retired with that qualifier (an admitted HAS_MEMBER row states HAS_PART {{meronymy/member}}).
CREATE OR REPLACE FUNCTION laplace.claim_has_qualifier(
        p_type bytea, p_mask bytea, p_family text, p_value text)
    RETURNS boolean
    LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $q$
    SELECT laplace.has_qualifier(p_mask, p_family, p_value)
        OR p_type = ANY (CASE p_family || '/' || p_value
{q_arms}
            ELSE ARRAY[]::bytea[]
        END)
$q$;
...
def main() -> int:
    ...
    emit_deprel_law()
    emit_qualifier_law(MANIFEST / "qualifiers.toml", {r["canonical"] for r in rel["relation"]}, rel["relation"])
