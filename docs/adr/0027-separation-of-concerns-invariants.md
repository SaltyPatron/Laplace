...
| **SQL / DbUp migrations** (`db/migrations/`) | Idempotent DDL; role grants; CREATE EXTENSION orchestration via the laplace_priv wrapper; ALTER DEFAULT PRIVILEGES | Business logic; procedural transforms; anything non-declarative; substrate schema definition (per [ADR 0023](0023-extension-owns-schema-dbup-orchestrates.md), schema lives in extension upgrade scripts) |
