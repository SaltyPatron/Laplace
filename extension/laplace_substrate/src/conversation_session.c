...
session_manifest(void)
{
    ...
    return session_plan(&session_manifest_plan,
        ...
        " WHERE p.id = $1 AND p.type = 1 AND p.trajectory IS NOT NULL", 1, types);
}
...
pg_laplace_session_append_turns(PG_FUNCTION_ARGS)
{
    ...
    SPIPlanPtr lock = session_plan(&session_lock_plan,
        "SELECT id FROM laplace.entities WHERE id = $1 FOR UPDATE", 1, id_types);
    ...
    SPIPlanPtr coordinates = session_plan(&session_coords_plan,
        ...
        " JOIN laplace.physicalities p ON p.entity_id = t.id AND p.type = 1"
        " ORDER BY t.ordinal, p.id", 1, coord_types);
    ...
    SPIPlanPtr write = session_plan(&session_write_plan,
        ...
        " observed_at = EXCLUDED.observed_at", 10, write_types);
