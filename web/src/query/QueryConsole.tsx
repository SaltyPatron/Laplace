...
export function QueryConsole() {
  ...
  const canRun = !!active && topic.length > 0 && (!needsTopic2 || topic2.length > 0) && (!needsType || relationType.length > 0);
  const resultRead = useReadResource({
    key: JSON.stringify(['query-result', tenant, quoteId]), enabled: false,
    read: async (signal) => {
      ...
      const result = await runQuery(request, { tenant, quoteId, signal });
      return { result, snapshot: captureRows(result.rows, `Rows returned by ${result.shape}; the submitted shape and dials bound this read. No unreturned graph content is included.`, request) };
    },
  });
  const result = resultRead.data?.result;
  const columns: ResultColumn<QueryRow>[] = [
    { key: 'reply', label: 'Reply' },
    { key: 'eff_mu', label: 'Standing', render: (row) => <ConsensusBadge mu={row.eff_mu ?? undefined} witnesses={row.witnesses ?? undefined} /> },
  ...
  useEffect(() => {
    if (!querySeed) return;
    setTopic(querySeed.topic); setTopic2(querySeed.topic2 ?? ''); setShape(querySeed.shape ?? 'describe');
    setRelationType(querySeed.relationType ?? ''); setSelectedBands(querySeed.bands ?? []);
    setQuerySeed(null); autoRun.current = true;
  }, [querySeed, setQuerySeed]);
  useEffect(() => {
    if (!autoRun.current || !canRun || querySeed != null) return;
    autoRun.current = false; void resultRead.reload();
  ...
  return <div className={styles.layout}>
    <section className={styles.controls} aria-label="Query controls">
      <Panel title="Ask"><form className={styles.stack} onSubmit={(event) => { event.preventDefault(); if (canRun && !resultRead.busy) void resultRead.reload(); }}>
      ...
      {usesBands && <Panel title="Lens"><ReadStatus label="Relation bands" resource={bandsRead} />
    ...
    <section className={styles.results} aria-label="Query results">
      <Panel title="Result" fill actions={result?.topic_id ? <RouterLink className={styles.entityLink} to={`/explore/entity/${result.topic_id}`}>open {result.topic_label} in Explore →</RouterLink> : null}>
        <ReadStatus label="Query result" resource={resultRead} />
        {resultRead.status === 'idle' && <div className={styles.empty}><h3>Ask the graph directly.</h3><p>Name a topic, choose an available read, and narrow it with the lens.</p><Muted>The shape and lens specify the read; this is not conversational interpretation.</Muted></div>}
        {result && resultRead.data && <>
          <div className={styles.resultHead}><span className={styles.resultTopic}>{result.topic_label}</span>
          ...
          <details><summary>Submitted query and dials</summary><pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{JSON.stringify(resultRead.data.snapshot.context, null, 2)}</pre></details>
          <ResultWorkspace scopeKey={JSON.stringify(['query', tenant, quoteId])} label="Query result rows" snapshot={resultRead.data.snapshot} columns={columns} />
        </>}
      </Panel>
    </section>
  </div>;
