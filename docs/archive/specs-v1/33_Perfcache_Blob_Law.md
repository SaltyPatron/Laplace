> Archived specification snapshot. Historical implementation table; not current status.
...
| `laplace_modality_number_perfcache.bin` | t0 + decimal digit strings for values **0..255** → `laplace_modality_number_tables_emit` (text content root / ScalarId law). **Not** packed-RGBA or PCM tier-0; tier 0 remains codepoints | `value → number id/coord` O(1) dense index (`modality_number_table_lookup`); image channel bytes + shared number floor for audio magnitudes in range | landing — format, native emit (always, no corpus), load API; higher image/audio tiers (pixel/patch/sample/…) and signed PCM scope TBD |
