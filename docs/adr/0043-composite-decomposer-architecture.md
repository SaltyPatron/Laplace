...
Cross-vendor consensus is built across the same mechanical-role attestation kinds; Llama-derived `Q_PROJECTS(walk → ed)` and Mistral-derived `Q_PROJECTS(walk → ed)` accumulate into one consensus row per `(subject, kind, object)` because the kind is mechanical, not vendor-named. The substrate's view of vendor naming itself is also consensus: when two vendors agree on a name's meaning, those attestations cluster; when they disagree, the dispute lives in substrate (`Llama_source attests "q_proj" means Q_PROJECTS; HuggingFace_transformers_default_source attests same with different rating`), resolvable via arena semantics rather than hardcoded plugin overrides.
