...
BEGIN
    ...
    VALUES
        ...
        (public.laplace_hash128_blake3('test/chess/ev_strong_source2'), pos, mv, n_strong, src2, g_white, 2, now(), 1, 1000000000, 30000000000),
...
BEGIN
    ...
    ON CONFLICT DO NOTHING;
    -- Admit an additional observed type through the canonical INSERT owner;
    -- the scalar compatibility summary is not the membership relation.
    INSERT INTO laplace.entities (id, tier, type_id, first_observed_by)
    SELECT id, 0, laplace.entity_type_id('Chess_Player'), src
    FROM unnest(ARRAY[tal, botv, spas]) AS players(id);
