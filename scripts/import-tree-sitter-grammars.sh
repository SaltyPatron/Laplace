...
#
# Strategy:
#   - For each tree-sitter-* dir under VAULT, extract origin URL
#   - git submodule add --reference <local-vault-path> <url> external/tree-sitter-grammars/<lang>
#   - --reference uses local objects (no network for the bulk of history)
...
for grammar_dir in "$VAULT"/tree-sitter-*; do
    ...
    if err_output=$(git submodule add --reference "$grammar_dir" "$url" "$submodule_path" 2>&1); then
        added=$((added + 1))
