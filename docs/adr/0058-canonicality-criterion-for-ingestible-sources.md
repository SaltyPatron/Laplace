...
| **(4) Information content** | Carries the source's full-fidelity values (the publisher's training output as-is), so the semantic facts dissolved out of it are the source's own | Lossy conversion produced it from a canonical source, so its values are approximations of the canonical's → DERIVED. Note: canonicality is about *whether the values are the source's own full-fidelity values*, NOT about bit-perfect preservation — bit-perfect preservation is a non-goal per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6; the substrate dissolves the artifact to semantic facts and discards the blob either way. |
...
An artifact is CANONICAL when heuristics (1)+(2)+(4) all hold (with (3) and (5) as supporting evidence). When any of (1), (2), or (4) fails, the artifact is DERIVED. (Heuristic (4) is about information content — full-fidelity source values vs lossy approximations — not about bit-perfect preservation, which is a non-goal per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6.)
...
| **GGUF** (per-quant Q4_K_M / Q5_K_M / Q6_K / Q8_0 / etc.) | DERIVED / lossy | **NO** | yes (proof / compat) | llama.cpp runtime / deployment optimization. Per [ADR 0037](0037-layered-seed-ingestion-and-model-codec-fidelity.md) GGUF is the substrate's proof export shape — synthesized from substrate state for chat-verification (not a preserved copy of any ingested blob); never an ingest source. |
...
| **PNG / lossless WebP / lossless AVIF / lossless HEIC** | CANONICAL | yes | yes (native) | Lossless image formats; decoded pixel values are the source's own full-fidelity values. |
...
| **FLAC / WAV / AIFF / lossless ALAC** | CANONICAL | yes | yes (native) | Lossless audio formats; decoded PCM samples are the source's own full-fidelity values. |
...
5. **Does NOT silently fall back to ingesting the derived as a degraded source.** No "ingest at lower trust" optimization is permitted at this layer — and note that trust is not a tier or class to dial down anyway: trust is a self-tuning Glicko-2 value emerging from cross-source agreement per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5. Quantization loss is not a credibility question; it is an information-content question, so reaching for a trust knob is category-confused (see Alternatives). The user can authorize an explicit override if they have explicit reason (e.g., the derived artifact is the only one ever published for a particular model), but that requires an explicit user-authorized override + the substrate records the override in its provenance trail.
...
- Set source trust. Source trust is a self-tuning Glicko-2 value (NOT a fixed tier or class — per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5); canonicality is a separate, per-artifact-per-source axis. Both apply independently — a canonical source's attestations carry whatever Glicko-2 trust emerges from cross-source agreement; a derived artifact from a well-rated source is still refused-as-knowledge regardless of how the source is rated, because the refusal is about information content, not about the source's standing. (Historical note: ADR 0044 framed source trust as a class taxonomy; that "TrustClass ladder" framing is superseded — trust is a Glicko-2 value, not a tier.)
...
- **No fallback ingest pathway** — operators encountering a derived-only-available case must explicitly authorize an override (recorded in the provenance trail). The default is refusal-with-existence-recording. (Trust is not the dial here — it is a self-tuning Glicko-2 value per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5, not a class to lower for a derived artifact.)
...
- **The dissolve-and-discard posture is consistent throughout**: the substrate consumes canonical artifacts (the developer's published full-fidelity values), dissolves them into typed Glicko-2 matchup attestations via [ADR 0056 ETL](0056-weight-tensor-etl-as-arena-matchup-observation.md), and discards the source blob per [ADR 0057 emission discipline](0057-substrate-emission-discipline-product-not-packaging.md) (bit-perfect preservation is a non-goal per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6). Derived artifacts wouldn't add knowledge value above what the canonical already provides; ingesting them would inflate substrate state with lossy approximations of facts the substrate already holds. (The "Food principle / Vampire mode" labels are just labels for this mechanism, not concepts in their own right — per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 10, state the mechanism, not the name.)

## Alternatives considered

- **Ingest derived artifacts at lower trust.** Rejected per the 2026-05-24 conversation: *"We dont care about quantized 'mp3' models."* Ingesting AWQ/GPTQ/etc. would inflate substrate state with attestations derived from lossy approximations of values the canonical source carries at full fidelity. Trust is the wrong dial — and trust is a self-tuning Glicko-2 value, not a class to set, per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5. Quantization loss isn't about the source's credibility; it's about the artifact's information content vs the canonical.
...
- **Ingest derived as a fallback when canonical isn't available, with degraded trust.** Rejected as default behavior — silent fallback creates substrate state that depends on which file the user happened to have available, not on the actual source's published canonical knowledge. An explicit user-authorized override (recorded in the provenance trail) is available for the rare case (e.g., a model never published in canonical form), but the default is refuse-with-existence-recording. (Trust is not a class to degrade — it is a self-tuning Glicko-2 value per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5.)
...
- [GLOSSARY Vampire mode](../../GLOSSARY.md) / [GLOSSARY Food principle](../../GLOSSARY.md) — labels for the dissolve-to-facts-and-discard-the-blob ingestion posture (the labels are not the concept — per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 10, the mechanism is what matters)
- [GLOSSARY Substrate Synthesis](../../GLOSSARY.md) — fresh emission from substrate state
- [DESIGN.md VII — Model dissolve/synthesis fidelity](../../DESIGN.md) (DESIGN's "codec" framing is a label, not the mechanism — per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 10; ingest dissolves to facts, synthesis pours facts into a chosen mold, no round-trip preservation)
...
- [ADR 0037](0037-layered-seed-ingestion-and-model-codec-fidelity.md) — layered seed ingestion (canonical safetensors is the model-ingest target; note seed-source attestations are OPTIONAL enrichment, semantic model ingest is the mandatory spine, per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6 — and the filename's "codec" is a label, not a round-trip-preservation claim)
...
- [ADR 0044](0044-attestation-kind-priors-and-source-trust-taxonomy.md) — source trust (orthogonal axis to canonicality; note its "trust-class taxonomy" framing is superseded — trust is a self-tuning Glicko-2 value per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 5)
