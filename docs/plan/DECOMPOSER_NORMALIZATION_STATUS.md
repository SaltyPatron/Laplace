...
| #1235 | Semantic frontier and synthetic-budget removal | Removes fixed prompt, cascade, contrast, taxonomy, and relation-rank truncation; sizes native result storage from the actual SPI population and PostgreSQL allocation ceiling; retains all source-selected TinyCodes keywords; and deletes unmeasured relation-summary verdict and disabled round-trip-budget thresholds. |
| #1236 | Glicko-derived exact traversal | Replaces the fitted edge-strength and A* logistic constants with Glicko-2 expected score and negative-log probability, removes the silent 256-neighbor expansion cap and implicit 7/20-hop defaults, and restricts geometry to exact equal-cost tie ordering. |
| #1237 | Folded-state confidence law | Removes the post-fold `exp(-rd/initial_rd)` and `witnesses/(witnesses+4)` transforms from walks, steering, and Foundry planes; all first-party paths now use signed Glicko expectation from rating/RD, and fresh/updated installs retire the obsolete knobs and overloads. |
| #1238 | Explicit ranking dimensions | Removes walk's `+2` geometry, `+3` topic, and transformed continuity score cocktail; ranks exact topic membership, ordinal distance, and angular distance only inside folded-belief ties. Prompt election now chooses direct coherence or exact structural specificity without the seed-sized `10^13` mass half-max, and every caller orders folded evidence separately instead of multiplying unrelated units. |
| #1239 | Direct type-routed fold batches | Type-major managed lanes send one scalar routing type per set operation instead of one duplicate type per row. Native passes original arrays directly into its cached literal-routed `MERGE`, constructs consensus IDs once, and retains a capability-cached legacy adapter for rolling deployment. |
| #1240 | Native fresh-cell fold source | Removes the record-returning `LATERAL` Glicko invocation from every consensus `MERGE` source row. Fresh-cell states are calculated in one tight native array pass using the same core fold, while matched cells continue from their locked stored state. |
| #1241 | Single working-set coordination owner | Removes redundant process-local entity/physicality/attestation claim dictionaries and their arbitrary 120-second spin-wait. The PostgreSQL advisory transaction lock remains the one apply owner, and every successor verifies committed presence after acquiring it. |
| #1242 | Exact operator SQL budgets | Removes the 600-second read and 21,600-second maintenance ceilings plus the conflicting hardcoded server timeout. Positive operator budgets are exact, zero is unbounded but cancellable, and negative budgets are rejected rather than silently rewritten. |
