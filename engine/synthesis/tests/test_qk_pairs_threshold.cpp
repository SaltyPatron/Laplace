...
/* Serial f64 reference running the identical compensated algorithm in fixed
 * order, emitting in (t ascending, s ascending) order. Ground truth the kernel
...
/* ── 3. Threshold behavior on a tiny case with known scores. ────────────────
 * d_model = head_dim = 2, Wq = Wk = identity, so q_t = E[t], k_s = E[s], and
 * score(t,s) = E[t]·E[s].
 *   E[0] = (2, 0), E[1] = (0, 3), E[2] = (1, 0)
 *   scores (t,s):
 *     (0,0)=4  (0,1)=0  (0,2)=2
 *     (1,0)=0  (1,1)=9  (1,2)=0
 *     (2,0)=2  (2,1)=0  (2,2)=1
 * floor = 1.5 keeps {4, 2, 9, 2} => pairs (0,0),(0,2),(1,1),(2,0). |score|<=floor
 * (the 1's and 0's) are excluded; >floor included. */
TEST(QkPairsThreshold, ThresholdBehaviorKnownScores) {
    ...
    std::vector<float> I = {1,0, 0,1};  /* identity for both Wq, Wk */
    std::vector<qk_pair_f64_t> out(64);
    int ov = -1;
    ...
    /* floor exactly at a score => strict >: |score|==floor excluded. floor=2
     * drops the two ==2 pairs, keeps 4 and 9. */
...
/* ── 2. Bitwise vs in-test serial f64 reference (identical algorithm). ─────── */
TEST(QkPairsThreshold, BitwiseMatchesSerialReference) {
    const size_t vocab = 137, d_model = 53, head_dim = 17;  /* off TBB grain */
...
/* ── 1. Determinism across thread counts: bitwise scores + identical set/order.
 * Also exercises windowing: full [0,vocab) must equal the concatenation of
 * consecutive sub-windows, proving window-independence. */
TEST(QkPairsThreshold, DeterministicAcrossThreadCountsAndWindows) {
    ...
    /* Windowed: process [0,57) + [57,131) + [131,200), concatenate. Must equal
     * the full run bit-for-bit (proves window-independence of the emitted set). */
...
/* ── 4. Overflow signaling: tiny out_cap => overflow=1, returned prefix is a
 * deterministic whole-row prefix matching the head of the full run. ────────── */
TEST(QkPairsThreshold, OverflowSignalsAndPrefixIsDeterministic) {
    ...
    /* Tiny cap: smaller than the full count, large enough for >=1 row. */
    ...
    /* The returned prefix must be the exact head of the full run, AND it must be
     * a whole number of leading query rows: the last emitted pair's query_idx
     * must differ from the first excluded pair's query_idx (no partial row). */
    ...
    /* cap = 0 => overflow, zero rows fit, returns 0. */
