...
**The restriction is not a technicality, and an earlier draft of this document
got it wrong.** `(rating − neutral)` is *signed by design*: the fold's position
relative to `glicko2_neutral_mu()` is the **verdict** — affirm above, reject
below, neutral at — and `attestations.outcome` carries the same semantics at
the evidence layer. A claim rated below neutral is testimony that it is
**false**, which is the distinction the read rules exist to protect ("an
unattested id is not an id attested false").

So:

- `log(rating − neutral)` **does not exist** on the reject half. The log-linear
  form is a description of the affirm half only.
- A signed factor in a **product** flips the sign of the whole weight. Two
  rejected factors multiply to an affirmation.
...
### 3.2 A signed quantity used as a denominator

`prompt_coherence` accumulates `total_mass += rank × eff` where
`eff = rating − 2·rd` **can be negative** (a refuted or thin edge), then
computes `specificity = coherence / total_mass`. [S]

Near zero the ratio explodes; below zero it flips sign, and the elector's first
key silently inverts. The existing guard is `total_mass > 0.0`, which catches
the zero case and **not** the small-positive or sign-flip cases.

**[C] — not yet measured.** The falsifying query: distribution of `total_mass`
per candidate on a seeded box, and whether any candidate carries a negative
`rank × eff` contribution large enough to matter. Run before trusting
