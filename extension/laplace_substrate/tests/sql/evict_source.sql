...
DECLARE
    ...
    src_w    bytea := source_id('EvictTestWitness');
    src_a    bytea := source_id('EvictTestAnalysis');
    marker_t bytea := entity_type_id('Evict_TestMarker');
    rel_p    bytea := laplace_hash128_blake3('test/evict/rel');
    rel_hot  bytea := relation_type_id('IS_A');
    subj     bytea := laplace_hash128_blake3('test/evict/subject');
    o1       bytea := laplace_hash128_blake3('test/evict/obj1');
    o2       bytea := laplace_hash128_blake3('test/evict/obj2');
    o3       bytea := laplace_hash128_blake3('test/evict/obj3');
    o4       bytea := laplace_hash128_blake3('test/evict/obj4');
    o5       bytea := laplace_hash128_blake3('test/evict/obj5');
    m1       bytea := laplace_hash128_blake3('test/evict/marker1');
    m2       bytea := laplace_hash128_blake3('test/evict/marker2');
    m3       bytea := laplace_hash128_blake3('test/evict/content-by-analysis');
    m4       bytea := laplace_hash128_blake3('test/evict/marker-of-witness');
...
BEGIN
    ...
    VALUES
        ...
        (laplace_hash128_blake3('test/evict/a1'), subj, rel_p, o1, src_a, NULL,
         2, t2, 2, 2 * s_conf, phi_a),
        (laplace_hash128_blake3('test/evict/a2'), subj, rel_p, o1, src_a, NULL,
         1, t3, 1, s_ref, phi_a),
        -- cell II (subj, rel_p, o2): MIXED — witness survives, cell refolds
        (laplace_hash128_blake3('test/evict/w1'), subj, rel_p, o2, src_w, NULL,
         2, t1, 3, 3 * s_conf, phi_w),
        (laplace_hash128_blake3('test/evict/a3'), subj, rel_p, o2, src_a, NULL,
         2, t2, 2, s_conf, phi_a),
        -- cell III (subj, rel_p, o3): WITNESS ONLY — untouched, byte-identical
        (laplace_hash128_blake3('test/evict/w2'), subj, rel_p, o3, src_w, NULL,
         2, t1, 4, 2 * win, phi_w),
        -- cell IV (subj, IS_A, o4): MIXED on a HOT (hash-partitioned) relation
        (laplace_hash128_blake3('test/evict/w3'), subj, rel_hot, o4, src_w, NULL,
         2, t1, 2, 2 * s_conf, phi_w),
        (laplace_hash128_blake3('test/evict/a4'), subj, rel_hot, o4, src_a, NULL,
         0, t2, 1, 0, phi_a),
        -- cell V (subj, rel_p, NULL): ANALYSIS ONLY, NULL object — culled
        (laplace_hash128_blake3('test/evict/a5'), subj, rel_p, NULL, src_a, NULL,
         2, t2, 3, 3 * win, phi_a),
        ...
        (laplace_hash128_blake3('test/evict/w4'), subj, rel_p, o5, src_w, NULL,
         2, t1, 1, win, phi_w),
        (laplace_hash128_blake3('test/evict/a6'), subj, rel_p, o5, src_a, NULL,
         2, t2, 2, 2 * s_conf, phi_a),
        (laplace_hash128_blake3('test/evict/w5'), subj, rel_p, o5, src_w, NULL,
         1, t3, 2, s_conf, phi_w);
    ...
    FROM consensus c WHERE c.subject_id = subj;
...
-- relation; p_drain = false leaves highway_mask_dirty observable (drain itself is
-- highway machinery with its own coverage, and its refresh warns when no highway
-- perfcache is configured — environment-dependent output a regress file cannot pin).
CALL evict_source(source_id('EvictTestAnalysis'), NULL,
                  ARRAY[entity_type_id('Evict_TestMarker')],
...
DECLARE
    ...
    t3       timestamptz := '2026-03-01 00:00:00+00';
    n        bigint;
    row_c    consensus%ROWTYPE;
    snap     _ev_snap_v1%ROWTYPE;
    expect   laplace_glicko2_result;
    step     laplace_glicko2_result;
    leafname text;
BEGIN
    -- every evidence row of the evicted source is gone; the witness layer is intact
    SELECT count(*) INTO n FROM attestations WHERE source_id = src_a;
    IF n <> 0 THEN RAISE EXCEPTION 'FAIL: % analysis rows survived eviction', n; END IF;
    SELECT count(*) INTO n FROM attestations WHERE source_id = src_w;
    ...
    -- attested-false)
    IF EXISTS (SELECT 1 FROM consensus WHERE subject_id = subj AND type_id = rel_p
    ...
    SELECT * INTO row_c FROM consensus
    ...
    expect := laplace_glicko2_accumulate_games(
        glicko2_neutral_mu(), glicko2_initial_rd(), glicko2_initial_volatility(),
        glicko2_neutral_mu(), phi_w, 3, 3 * s_conf, glicko2_tau());
    SELECT * INTO row_c FROM consensus
    ...
    expect := laplace_glicko2_accumulate_games(
        glicko2_neutral_mu(), glicko2_initial_rd(), glicko2_initial_volatility(),
        glicko2_neutral_mu(), phi_w, 2, 2 * s_conf, glicko2_tau());
    SELECT * INTO row_c FROM consensus
    ...
    END IF;
    SELECT c.tableoid::regclass::text INTO leafname FROM consensus c
    ...
    step := laplace_glicko2_accumulate_games(
        glicko2_neutral_mu(), glicko2_initial_rd(), glicko2_initial_volatility(),
        glicko2_neutral_mu(), phi_w, 1, win, glicko2_tau());
    expect := laplace_glicko2_accumulate_games(
        step.rating, step.rd, step.volatility,
        glicko2_neutral_mu(), phi_w, 2, s_conf, glicko2_tau());
    SELECT * INTO row_c FROM consensus
    ...
    IF EXISTS (SELECT 1 FROM entities WHERE id IN (m1, m2)) THEN
        RAISE EXCEPTION 'FAIL: analysis markers survived eviction';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM entities WHERE id = m3) THEN
        RAISE EXCEPTION 'FAIL: content entity deleted — content is never evicted';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM entities WHERE id = m4) THEN
    ...
    SELECT count(*) INTO n FROM highway_mask_dirty
    WHERE id IN (subj, o1, o2, o4, o5);
    IF n <> 5 THEN
        RAISE EXCEPTION 'FAIL: mask queue holds % of the 5 touched entities', n;
    END IF;
    IF EXISTS (SELECT 1 FROM highway_mask_dirty WHERE id = o3) THEN
...
CALL evict_source(source_id('Evict_NoSuchSource'), NULL, NULL,
                  p_drain => false);
...
BEGIN
    ...
    IF n <> 4 OR (SELECT count(*) FROM consensus WHERE subject_id = subj) <> 4 THEN
...
BEGIN
    INSERT INTO attestations
        (id, subject_id, type_id, object_id, source_id, context_id,
         ...
         sum_score_fp1e9, opponent_rd_fp1e9)
    VALUES
        (laplace_hash128_blake3('test/evict/v2/a1'), subj, rel_p, o1, src_a, NULL,
         2, t2, 2, 2 * s_conf, phi_a),
        (laplace_hash128_blake3('test/evict/v2/a2'), subj, rel_p, o1, src_a, NULL,
         1, t3, 1, s_ref, phi_a),
        (laplace_hash128_blake3('test/evict/v2/a3'), subj, rel_p, o2, src_a, NULL,
         2, t2, 2, s_conf, phi_a),
        (laplace_hash128_blake3('test/evict/v2/a4'), subj, rel_hot, o4, src_a, NULL,
         0, t2, 1, 0, phi_a),
        (laplace_hash128_blake3('test/evict/v2/a5'), subj, rel_p, NULL, src_a, NULL,
         2, t2, 3, 3 * win, phi_a),
    ...
    JOIN _ev_snap_v1 s
     ...
     AND s.witness_count = c.witness_count
    WHERE c.subject_id = subj;
    IF n <> 6 OR (SELECT count(*) FROM consensus WHERE subject_id = subj) <> 6 THEN
...
DECLARE
    ...
    subj  bytea := laplace_hash128_blake3('test/evict/subject');
BEGIN
    DELETE FROM attestations WHERE source_id IN (src_w, src_a);
    DELETE FROM consensus WHERE subject_id = subj;
    DELETE FROM highway_mask_dirty WHERE id IN (
        subj,
        laplace_hash128_blake3('test/evict/obj1'),
        laplace_hash128_blake3('test/evict/obj2'),
        laplace_hash128_blake3('test/evict/obj4'),
        laplace_hash128_blake3('test/evict/obj5'));
    DELETE FROM entities WHERE id IN (
        src_w, src_a,
        laplace_hash128_blake3('test/evict/rel'), subj,
        laplace_hash128_blake3('test/evict/obj1'),
        laplace_hash128_blake3('test/evict/obj2'),
        laplace_hash128_blake3('test/evict/obj3'),
        laplace_hash128_blake3('test/evict/obj4'),
        laplace_hash128_blake3('test/evict/obj5'),
        laplace_hash128_blake3('test/evict/marker-of-witness'),
        laplace_hash128_blake3('test/evict/content-by-analysis'));
