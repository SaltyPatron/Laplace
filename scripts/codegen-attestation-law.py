...
def emit_relation_law(rel: dict) -> None:
    ...
    #
    # A retirement names how its meaning is carried now; the retired name itself resolves to
    # nothing, and a source's own surface for that meaning is an [[alias]]:
    ...
    for r in relations:
        if not r.get("retired"):
            for field in ("parent", "family_root"):
                if r.get(field) in retired:
                    raise SystemExit(f"relation {r['canonical']} has retired {field} {r[field]}")
    ...
    # A retired name is not a surface: emission through it fails closed. Its bit stays
    # reserved (append-only) and its retired/retired_flip/retired_qualifier fields record
    # which element and qualifier carry its meaning now.
