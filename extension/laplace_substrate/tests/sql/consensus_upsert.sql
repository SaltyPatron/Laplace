...
DECLARE
    ...
    src      bytea := laplace_hash128_blake3('test/upsert/source');
    rel_a    bytea := laplace_hash128_blake3('test/upsert/rel_a');
    rel_hot  bytea := relation_type_id('IS_A');
    subj     bytea := laplace_hash128_blake3('test/upsert/subject');
    o1       bytea := laplace_hash128_blake3('test/upsert/obj1');
    o2       bytea := laplace_hash128_blake3('test/upsert/obj2');
    ...
    affected bigint;
    row1     consensus%ROWTYPE;
    row2     consensus%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO row1 FROM consensus
    ...
    expect := laplace_glicko2_accumulate_games(
        ...
        glicko2_neutral_mu(), phi, 2, 2 * s_conf, glicko2_tau());
    ...
    SELECT * INTO row2 FROM consensus
    ...
    expect := laplace_glicko2_accumulate_games(
        ...
        glicko2_neutral_mu(), phi, 5, 5 * s_conf, glicko2_tau());
    affected := consensus_upsert(
