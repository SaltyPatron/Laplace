...
BEGIN
    ...
    SELECT array_agg(d.id ORDER BY u.ord), array_agg(d.label ORDER BY u.ord)
      INTO ids, labels
    FROM unnest(ARRAY[concept, doc, sent1, wolf, opaque]) WITH ORDINALITY u(id, ord)
    JOIN LATERAL realize.display_label_batch(ARRAY[u.id]) d ON true;

    IF ids IS DISTINCT FROM ARRAY[concept, doc, sent1, wolf, opaque] THEN
        RAISE EXCEPTION 'FAIL: display label changed/reordered entity identity';
    END IF;
    IF labels[1] IS DISTINCT FROM '狼。' THEN
        RAISE EXCEPTION 'FAIL: opaque concept did not use its definition preview: %', labels[1];
    END IF;
    IF labels[2] IS DISTINCT FROM '狼。' THEN
        RAISE EXCEPTION 'FAIL: definition document did not use containment-proven first-sentence preview: %', labels[2];
    END IF;
    IF labels[3] IS DISTINCT FROM '狼。' THEN
        RAISE EXCEPTION 'FAIL: sentence content did not render Unicode exactly: %', labels[3];
    END IF;
    IF labels[4] IS DISTINCT FROM '狼' THEN
        RAISE EXCEPTION 'FAIL: tier-0 codepoint did not render Unicode exactly: %', labels[4];
    END IF;
    IF labels[5] ~ '^[0-9A-Fa-f]{32}(…|\.\.\.)?$' THEN
        RAISE EXCEPTION 'FAIL: unresolved opaque entity leaked identity as label: %', labels[5];
    END IF;
    IF labels[5] IS NULL OR btrim(labels[5]) = '' THEN
    ...
    SELECT array_agg(d.label ORDER BY u.ord)
      INTO labels
    FROM unnest(ARRAY[concept, opaque, concept]) WITH ORDINALITY u(id, ord)
    JOIN LATERAL realize.display_label_batch(ARRAY[u.id]) d ON true;
