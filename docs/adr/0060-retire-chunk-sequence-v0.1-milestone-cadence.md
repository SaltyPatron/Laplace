...
**3. Forward work tracks against the `v0.1 — model round-trip` milestone**, not chunk numbers. The decisive v0.1 deliverable per [ADR 0037](0037-layered-seed-ingestion-and-model-codec-fidelity.md) is **one source-model round-trip** (model → substrate → native package → GGUF proof export → chat). This round-trip is NOT bit-perfect preservation and is NOT a "codec" — bit-perfect preservation is worthless (it only returns the file you already had); the model is dissolved to typed, sourced, Glicko-2-rated attestations and the blob is discarded, then synthesis **pours those substrate facts back into the source's own mold** as one shape among many the same machinery can fill (per docs/SUBSTRATE-FOUNDATION.md truths 6 and 10). "Faithful" here means the materialized facts produce coherent behavior, not that any bytes are recovered. Its critical path is on the milestone: #6 (TransformerModelSource + Procrustes + sparsity), #191 (ModelDecomposer), #7 (synthesis → GGUF), #8 (round-trip + chat), #50 (determinism gate), #181 (ADR 0037 acceptance). The broader seed-source attestations (WordNet/OMW/UD/Wiktionary/Tatoeba/ConceptNet/Atomic2020, #184–#194) are **OPTIONAL enrichment, not v0.1-blocking** (CLAUDE.md hard rule 9); semantic ingest of the model alone is the mandatory spine.

> Note: the linked ADR's filename retains the word "codec" for path stability, but that label is banned as a concept per docs/SUBSTRATE-FOUNDATION.md truth 10 — model ingestion is a streaming O(params) ETL of weight cells into Glicko-2 matchup observations (truth 1), not a reversible codec. The interior `d×d` tensor-axis → token-entity resolution that this round-trip depends on (for `q/k/v/o/gate/up/down`, as opposed to the directly token-anchored `embed_tokens`/`lm_head`) is **OPEN per docs/SUBSTRATE-FOUNDATION.md** — not settled by this milestone framing.
