...
set "EXT_AVAIL="
for /f "usebackq delims=" %%v in (`psql -h %LAPLACE_PGHOST% -U %LAPLACE_PGUSER% -d postgres -tAc "SELECT default_version FROM pg_available_extensions WHERE name='laplace_substrate'"`) do set "EXT_AVAIL=%%v"
...
set "PENDING="
for /f "usebackq delims=" %%n in (`psql -h %LAPLACE_PGHOST% -U %LAPLACE_PGUSER% -d postgres -tAc "SELECT count(*) FROM pg_settings WHERE pending_restart"`) do set "PENDING=%%n"
if not "%PENDING%"=="0" (
