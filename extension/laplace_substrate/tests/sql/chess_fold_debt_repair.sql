...
BEGIN
    ...
    IF r.rating = 10398275800000000 THEN
        RAISE EXCEPTION 'FAIL: runaway rating survived full chess refold';
    END IF;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'FAIL: second chess rating repair changed an already repaired cell';
    END IF;
    RAISE NOTICE 'full chess rating repair replay is idempotent';
END $$;
