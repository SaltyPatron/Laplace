...
"""
    ...
    # one-partition-per-relation layout (~340 leaves/table) was measured to tax
    # planning and TRUNCATE/ANALYZE far more than the tail's row counts justify
    # (regress: converse 0.3s -> 54s, fold 1.5s -> 361s). Partition capacity
    # follows traffic. Emitted as ONE plpgsql block so the hot list stays
    # single-sourced in relation_types.toml; skips legacy non-partitioned tables
    # (greenfield law).
    #
    # ADOPTION (2026-07-26): the roster used to be frozen at greenfield. The block
    # only ever CREATEd missing partitions, and PG refuses to add a LIST partition
    # whose rows already sit in a non-empty DEFAULT ("updated partition constraint
    # for default partition would be violated by some row") — so promoting a
    # relation to hot on a populated DB required db-reset + full reseed, and in
    # practice never happened. MEASURED consequence: Tatoeba's two dominant
    # relations (HAS_EXTERNAL_ID, IS_TRANSLATION_OF) rode consensus_rdefault to
    # 5.2 GB / 9.5M rows — the largest consensus table in the DB, one heap and one
    # btree taking every commit worker's writes, with LWLock.BufferContent /
    # WALInsert contention and input rate decaying 8,190 -> 5,093 rec/s mid-run.
    # So the block now ADOPTS: detach the default, create every missing partition
    # against a defaultless parent (no validation scan), drain the matching rows
    # back through the parent's router, reattach. One detach/attach per table for
    # the whole batch, and only when something is actually missing — a no-op
    # install still takes no lock. This is what makes `hot` a maintainable roster
    # instead of a one-shot greenfield guess.
    hot_names = sorted({r["canonical"] for r in relations if r.get("hot") is True})
    part_lines = [
        "-- GENERATED by scripts/codegen-attestation-law.py — do not edit.",
    ...
    for i, n in enumerate(hot_names):
        part_lines.append(f"        '{n}'" + ("," if i < len(hot_names) - 1 else ""))
    part_lines += [
        "    ];",
        ...
        "    opts text := ' SET (autovacuum_analyze_scale_factor = 0.02,"
        ...
        "                         WHERE to_regclass(current_schema() || '.' || tbl"
        " || '_r_' || lower(h)) IS NULL);",
        ...
        "            FOR i IN 0..7 LOOP",
        "                EXECUTE format('CREATE TABLE %I PARTITION OF %I FOR VALUES"
        " WITH (MODULUS 8, REMAINDER %s)', part || '_h' || i, part, i);",
        ...
        "            RAISE NOTICE 'laplace: adopted % relation(s) into % partitions"
        " (% rows drained from %)', cardinality(missing), tbl, moved, dflt;",
        "        END IF;",
        "    END LOOP;",
        "END $$;",
    ]
    part_frag = OUT_SEED_FRAG.parent / "seed_relation_partitions.sql.in"
    _write_text_if_changed(part_frag, "\n".join(part_lines) + "\n")
