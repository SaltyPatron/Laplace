...
/// <summary>SAN → legal-move resolution, verified by replaying whole real games. Pure C#, no native/DB.</summary>
public sealed class SanTests
{
    ...
    {
        // Anthony-Hart vs baseball_astar2, chess.com 2026-05-14, 1-0 (mate on Qf7#).
        const string game =
        ...
        var m = new ChessModality();
        Assert.Equal(GameOutcome.WonBy(0), m.Terminal(end)); // white wins by checkmate
    }

    [Theory]
    [InlineData("e4 e5 Nf3 Nc6 Bb5", 5)]          // Ruy Lopez: pawns, knights, bishop
    [InlineData("e4 c5 Nf3 d6 d4 cxd4 Nxd4 Nf6 Nc3 a6", 10)] // Najdorf: captures + disambig-free
    [InlineData("d4 d5 c4 e6 Nc3 Nf6 Bg5 Nbd7", 8)] // QGD: Nbd7 needs file disambiguation
    ...
    {
        // White O-O and Black O-O-O reachable after these moves.
