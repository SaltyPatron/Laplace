...
**tree-sitter grammars** (303 `tree-sitter-<lang>` repos) ARE bulk-submoduled under `external/tree-sitter-grammars/<lang>/`. Each grammar is C source code (`grammar.js` → generated `src/parser.c` via tree-sitter CLI → compiled `<lang>.so` parser loaded at runtime by libtree-sitter). They are direct C source dependencies — same submodule policy as PostgreSQL/BLAKE3/etc.

Bulk-add is performed via `scripts/import-tree-sitter-grammars.sh`, which for each `/vault/Data/TreeSitter/tree-sitter-<lang>/` reads the upstream URL from its `origin` remote and issues `git submodule add <upstream-url> external/tree-sitter-grammars/<lang>`. Fresh clone from upstream (not `--reference` against /vault — those clones are shallow; git rejects shallow refs as a reference backstop). ~5-15 min one-time for 303 small repos. Pinning to specific release tags happens per-grammar when each language is wired into a `CodeDecomposer` (Chunk 7+).

`.gitmodules` grows by 303 entries (~50KB) — acceptable for full reproducibility. **Init is opt-in per grammar** — most contributors only init the language they're working on (`git submodule update --init external/tree-sitter-grammars/tree-sitter-python`); a full `git submodule update --init --recursive` is a one-time fetch.
