...
SELECT
 (SELECT count(DISTINCT subject_id) FROM consensus WHERE type_id=relation_type_id('IS_A')
    AND object_id=realize.canonical_id('WordNet_Synset'))   AS synsets,
 (SELECT count(DISTINCT subject_id) FROM consensus WHERE type_id=relation_type_id('IS_A')
    AND object_id=realize.canonical_id('WordNet_Sense'))    AS senses,
 (SELECT count(DISTINCT subject_id) FROM consensus WHERE type_id=relation_type_id('IS_A')
    AND object_id=realize.canonical_id('VerbNet_Class'))    AS verbnet_classes,
 (SELECT count(DISTINCT subject_id) FROM consensus WHERE type_id=relation_type_id('IS_A')
    AND object_id=realize.canonical_id('PropBank_Roleset')) AS propbank_rolesets,
 (SELECT count(DISTINCT subject_id) FROM consensus WHERE type_id=relation_type_id('IS_A')
    AND object_id=realize.canonical_id('FrameNet_Frame'))   AS framenet_frames;

\echo '== synset: identity reads back as the ILI (supermodel -> i93445); WN+OMW share the anchor =='
SELECT realize.render(sn.synset_id) AS ili_readback FROM lexical.senses(word_id('supermodel')) sn;
SELECT count(*) AS synsets_with_wn_and_omw FROM (
  SELECT s.subject_id FROM consensus s
  WHERE s.type_id=relation_type_id('IS_A') AND s.object_id=realize.canonical_id('WordNet_Synset')
...
SELECT count(*) AS converged_classes FROM (
  SELECT s.subject_id FROM consensus s
  WHERE s.type_id=relation_type_id('IS_A') AND s.object_id=realize.canonical_id('VerbNet_Class')
...
SELECT count(*) AS verbnet_to_wordnet_via_sense FROM (
  SELECT s.subject_id FROM consensus s
  WHERE s.type_id=relation_type_id('IS_A') AND s.object_id=realize.canonical_id('WordNet_Sense')
...
SELECT realize.render(e.type_id) AS entity_type, count(*) AS content_free
  FROM entities e
