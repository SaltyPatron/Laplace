...
| Early result reduction and honest top-k | Partial | `cluster`, `bubble_up`, `surface_sample`, angular KNN/locale, global top relations, band facts, evidence/salience reads, and vocabulary heads no longer hide guessed candidate multipliers. Rendering is total where identifiers are valid output; vocabulary lexical filters consume complete ranked populations through set-based pages. Read/foundry APIs preserve declared caps including zero. Remaining fixed limits and transport/operator caps still require classification. |
