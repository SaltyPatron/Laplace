...
function Cloud({p,pick}:{p:Float32Array;pick:(i:number)=>void}){const g=useMemo(()=>{const x=new THREE.BufferGeometry();x.setAttribute('position',new THREE.BufferAttribute(p,3));return x;},[p]);useEffect(()=>()=>g.dispose(),[g]);return <><points geometry={g} onClick={e=>{e.stopPropagation();if(e.index!=null)pick(e.index);}}><pointsMaterial size={1.15} sizeAttenuation={false} color="#e2e8f0" transparent={false} toneMapped={false}/></points><mesh><sphereGeometry args={[1,32,32]}/><meshBasicMaterial wireframe transparent opacity={.28} color="#94a3b8" toneMapped={false}/></mesh><OrbitControls makeDefault enablePan enableZoom/></>;}
