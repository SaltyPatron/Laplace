...
- Runner identity is conflated with the developer's interactive user (`ahart`).
- Sudo for `make install` requires the developer's password — blocking automated extension installs in CI.

## Why superseded

`ahart` is the developer's interactive identity. Using it for the CI runner conflates roles and ties CI operations to the developer's personal sudo. The runner should be a dedicated system account with bounded permissions. See [ADR 0019](0019-laplace-runner-system-account.md).
