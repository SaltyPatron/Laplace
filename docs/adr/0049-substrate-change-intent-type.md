...
- **Observability primitives attach to one type.** Per-intent metrics (entity count, physicality count, attestation count, novel-vs-existing ratio, transaction time) emit against `SubstrateChangeMetadata.SourceContentUnitName`. One dashboard tells you what's happening regardless of source. (No "round-trip" metric at ingest: ingest dissolves a source to semantic facts and discards the blob — bit-perfect preservation is worthless per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6. Round-trip vs. retarget is a *synthesis*-shape choice, not an ingest-tracked quantity.)
