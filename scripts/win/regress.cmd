...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$sArgs=@('--bindir='+[char]34+$pgbin+[char]34,'--host=localhost','--user=postgres','--inputdir=extension\laplace_substrate\tests','--outputdir='+$subOut,'--dbname=laplace_regress_substrate','--use-existing','bootstrap','glicko2_aggregate','entities_exist_bitmap','consensus_signed','consensus_period','consensus_fold','generation_corpus','converse','word_law','identity_law','schema_law','structural_surface','constituent_edges');" ^
...
"%PGREGRESS%" --bindir="%PGBIN%" --host=localhost --user=postgres ^
  ...
  bootstrap glicko2_aggregate entities_exist_bitmap consensus_signed consensus_period consensus_fold generation_corpus converse word_law identity_law schema_law structural_surface constituent_edges
