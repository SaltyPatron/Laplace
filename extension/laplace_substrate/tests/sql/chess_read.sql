...
BEGIN
    ...
    -- chess_game_plies: reads a game's line from its TRAJECTORY, so ordering comes from
    -- the packed geometry rather than from a replay. The packer is native (Trajectory.Build)
    -- and has no SQL-side builder, so a synthetic line cannot be fabricated here without
    -- reimplementing the mantissa packing -- a second implementation of a fact, which is
    -- exactly what is not wanted. What regress CAN pin is the contract that matters most:
    -- a game with no stored line returns nothing rather than guessing an order. The full
    -- decode is exercised against real games (ChessGameTrajectoryTests pins that a composed
    -- trajectory inverts to the exact position sequence).
