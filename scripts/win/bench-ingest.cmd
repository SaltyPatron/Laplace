...
rem Measures the dedup-once path (LAPLACE_BULK_FRESH=0). Default slice: 50k units.

rem

rem Usage:

rem   bench-ingest.cmd --confirm <source> [options]
...
rem   --confirm          REQUIRED — without this, prints plan and exits

rem   --full             No LAPLACE_INGEST_MAX_UNITS cap (entire corpus — YOU asked for this)

rem   --max-units N      Override unit cap (default 50000)

rem   --cold-only        Skip warm re-run

rem   --warm-only        Skip cold + DB isolate (re-run on existing laplace_bench)

rem   --no-isolate       Use laplace_bench without DROP/CREATE

rem   --no-build         Skip dotnet build

rem   --path PATH        Override ingest path

rem   --db NAME          Bench database (default laplace_bench; never laplace)

rem   --dry-run          Print plan only; no build, no DB, no ingest
...
if "%CONFIRMED%"=="0" (
  ...
  echo   Example slice:  bench-ingest.cmd --confirm wiktionary-en

  echo   Example full:   bench-ingest.cmd --confirm --full conceptnet

  echo   Plan only:      bench-ingest.cmd --dry-run --confirm ud
...
rem Pipeline path — never bulk-fresh for this benchmark.
...
if "%DO_FULL%"=="1" (
  ...
  set "LAPLACE_INGEST_MAX_UNITS=%MAX_UNITS%"
...
echo  LAPLACE INGEST BENCHMARK (pipeline / dedup-once)

echo  source=%SOURCE%  db=%BENCH_DB%  max_units=%MAX_UNITS%
...
if "%DO_WARM%"=="1" (
  ...
  echo ==== WARM RUN (--force re-ingest, bulk-fresh OFF) ====
...
if exist "%RUN_DIR%\cold.metrics" type "%RUN_DIR%\cold.metrics"

if exist "%RUN_DIR%\warm.metrics" type "%RUN_DIR%\warm.metrics"
...
if /i "%INGEST_STEP%"=="conceptnet" (
  ...
  if not defined LAPLACE_INGEST_COMPOSE_WORKERS set "LAPLACE_INGEST_COMPOSE_WORKERS=8"
...
echo LAPLACE_ROOT=%LAPLACE_ROOT%
...
echo LAPLACE_APPLY_PARTITIONS=%LAPLACE_APPLY_PARTITIONS%

echo LAPLACE_INGEST_BATCH=%LAPLACE_INGEST_BATCH%

echo LAPLACE_INGEST_COMMIT_ROWS=%LAPLACE_INGEST_COMMIT_ROWS%

echo LAPLACE_INGEST_WORKERS=%LAPLACE_INGEST_WORKERS%

echo LAPLACE_INGEST_COMPOSE_WORKERS=%LAPLACE_INGEST_COMPOSE_WORKERS%

echo LAPLACE_COMMIT_LANES=%LAPLACE_COMMIT_LANES%
...
echo command: dotnet run --project Laplace.Cli\Laplace.Cli.csproj -c Release --no-build -- ingest %INGEST_CLI_SOURCE% "%BENCH_PATH%" %INGEST_LANGS% %EXTRA_FLAGS% > "%LOG_FILE%.cmd"

echo wall_start=%DATE% %TIME% >> "%LOG_FILE%.cmd"

powershell -NoProfile -Command "$sw=[Diagnostics.Stopwatch]::StartNew(); & dotnet run --project Laplace.Cli\Laplace.Cli.csproj -c Release --no-build -- ingest %INGEST_CLI_SOURCE% \"%BENCH_PATH%\" %INGEST_LANGS% %EXTRA_FLAGS% 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; $sw.Stop(); 'WALL_CLOCK_SEC=' + [math]::Round($sw.Elapsed.TotalSeconds,2) | Tee-Object -FilePath '%LOG_FILE%' -Append"

set "RC=%ERRORLEVEL%"

echo wall_end=%DATE% %TIME% exit=%RC% >> "%LOG_FILE%.cmd"
