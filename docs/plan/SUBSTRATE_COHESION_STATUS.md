...
| Early result reduction and honest top-k | Partial | Numeric caps, unordered limits, fences, and post-join limits are inventoried. `cluster`, `bubble_up`, and `surface_sample` obey exact caller bounds; angular KNN has exact index order and no hidden candidate multiplier; other relation/read caps remain queued. |
