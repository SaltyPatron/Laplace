...
export function ReceiptPanel() {
  ...
  if (collapsed) {
    return (
      <aside className={styles.collapsed} aria-label="Evidence panel collapsed">
    ...
    <aside className={styles.panel} style={{ width }}>
      ...
      <div className={styles.body}>
        <div className={styles.heading}>
          <h2>Evidence</h2>
          <Button variant="ghost" size="sm" onClick={() => setCollapsed(true)}>Hide</Button>
        </div>
        ...
        {evidence && (
          <div className={evidenceStyles.evidence}>
            <header className={evidenceStyles.header}>
              <div>
                <h3><EntityLink idHex={evidence.entity_id} label={evidence.entity_label} /></h3>
                <Tooltip>
              ...
              <div className={evidenceStyles.headerActions}>
                ...
                <Button type="button" variant="ghost" size="sm" onClick={() => void copyText(evidence.entity_id)}>
                  Copy id
                </Button>
              </div>
            </header>

            {evidence.evidence.length === 0 && <Muted>No provenance-backed claims yet.</Muted>}
                      ...
                      return (
                        <tr key={`${item.type_id}-${item.object_id}`}>
                          ...
                          <Td className={evidenceStyles.sources}>
                            {sources.length > 0
                              ? sources.map((s) => <Chip key={s} variant="source">{s}</Chip>)
                              : '—'}
                          </Td>
