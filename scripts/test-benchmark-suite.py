...
class BenchmarkSuiteTests(unittest.TestCase):
    ...
    def test_workflow_is_dispatch_only_and_exposes_source_core_suites(self):
    ...
    def test_workflow_holds_real_host_lock_across_checkout_build_and_measurement(self):
        text = (ROOT / ".github/workflows/benchmark-evidence.yml").read_text(encoding="utf-8")
        lock = text.index('exec 9>"$lock_root/host-resource.lock"')
        target_fetch = text.index('git fetch --no-tags --prune origin "$target"')
        checkout = text.index('git checkout --no-overwrite-ignore --detach "$resolved"')
        execute = text.index('exec bash "$benchmark_driver"')
        self.assertEqual([lock, target_fetch, checkout, execute],
                         sorted([lock, target_fetch, checkout, execute]))
        self.assertNotIn("\nconcurrency:\n", text)
        self.assertIn('git show "$workflow_sha:scripts/benchmark-evidence-ci.sh"', text)

    def test_workflow_driver_binds_exact_built_core_and_t0(self):
        ...
        self.assertIn('export LAPLACE_CORE="$core"', text)
        self.assertIn('export LAPLACE_T0="$t0"', text)
        self.assertIn('export LAPLACE_PERFCACHE_BIN="$t0"', text)
