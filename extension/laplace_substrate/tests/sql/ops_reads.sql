...
SELECT count(*) = 1 AS modality_one_row FROM laplace.modality_counts();

SELECT count(*) >= 0 AS roster_runs
FROM laplace.source_roster(public.laplace_hash128_blake3('test/ops/source'), 3);
...
FROM laplace.mesh_position(laplace.word_id('x'));
...
FROM laplace.taxonomy_tree(laplace.word_id('x'));

SELECT count(*) >= 0 AS leaders_runs FROM laplace.band_leaders(ARRAY[1,2], 2);

SELECT count(*) = 1 AS record_one_row,
       bool_and(confirmed = 0 AND contested = 0 AND refuted = 0 AND thin = 0)
         AS record_zero_on_empty
FROM laplace.entity_record(laplace.word_id('x'));

-- the display-mu overload: fp1e9 -> display, one definition
SELECT laplace.eff_mu_display(1500000000000::bigint) = 1500.000 AS fp_display_scales;
...
VALUES
    (public.laplace_hash128_blake3('test/ops/bootstrap/law'),
     ...
     public.laplace_hash128_blake3('test/ops/source'), NULL, 2,
     statement_timestamp(), 1, 1000000000, 350000000000, NULL);
SELECT laplace.source_bootstrap_present(public.laplace_hash128_blake3('test/ops/source'),
                                laplace.relation_type_id('HAS_NAME_ALIAS')) AS bootstrap_present,
       NOT laplace.source_bootstrap_present(public.laplace_hash128_blake3('test/ops/source-unwitnessed'),
...
FROM laplace.ingest_run_close('00000000-0000-0000-0000-000000000001');
DO $$
BEGIN
    PERFORM laplace.ingest_run_close('00000000-0000-0000-0000-000000000001');
