    ...
    : DecomposerMultiPhase<UnicodeSource, FullScope>, IIngestInventoryProvider
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        {
            IDecomposer phase = CreateArtifactPhase(job, batch);
    ...
    private IDecomposer CreateArtifactPhase(ArtifactJob job, int batch) =>
        job.Kind switch
        {
    ...
    private sealed class DucetTier0Phase : UnicodeComposePhase<int>
    {
        ...
        protected override void Compose(int cp, SubstrateChangeBuilder builder)
        {
            ...
            // ASCII byte identities are exactly the codepoint identities just staged above;
            // they must retain DUCET/codepoint placement and type. ByteAtoms begins at 0x80.
            if (cp is >= ByteAtoms.First and <= 0xFF) EmitByte(builder, (byte)cp);
        }
