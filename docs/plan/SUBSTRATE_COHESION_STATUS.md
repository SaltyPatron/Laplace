...
| Early result reduction and honest top-k | Partial | Numeric caps, unordered limits, fences, and post-join limits are inventoried. `cluster`, `bubble_up`, and `surface_sample` now obey exact caller bounds at their natural grain; the remaining hidden candidate multipliers are still queued. |
