...
`scripts/measure-installed-chess-corpus.py` owns the separate `--mode inventory` and `--mode measure` invocations. Both require `--expected-source` and a new `--output-dir`; measurement accepts an observed `--pgn` or a configured `--family`. The former `chess-corpus-evidence.yml` dispatcher has been removed. The retained native operator calls this same measurement owner after actual installation and installed-database verification; ordinary main pushes do not select a measurement. Inventory lists actual configured Lumbras OTB, elite and TWIC files within bounded directory/file limits. Compressed files remain visible and ineligible. Directory symlinks are not traversed. Automatic measurement selection requires exactly one observed nonempty plain PGN in the selected OTB-2025 or elite family; an explicit observed absolute path can select another existing plain file.
