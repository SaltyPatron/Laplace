...
# The SQL fragments the attestation-law generator writes for the extension.
SEEDS = ("relation_family_ids.sql.in", "relation_set_ids.sql.in")
...
class ConfigureGeneration(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copytree(ROOT / "engine/manifest", self.root / "engine/manifest", dirs_exist_ok=True)
        ...
        ext_start = extension.index('# The extension version is content-derived')
        ...
        (self.root / "extension/laplace_substrate/CMakeLists.txt").write_text(
            ...
            + "".join(f' "${{CMAKE_CURRENT_SOURCE_DIR}}/sql/generated/{name}"\n' for name in SEEDS)
            + ')\n'
    ...
    def test_ninja_build_and_manifest_change_reconfigure_both_owners(self):
        ...
        result = subprocess.run(["cmake", "--build", str(self.build)],
                                text=True, capture_output=True, timeout=45)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        # A declared relation set changes a generated fragment, and so the extension version.
        manifest = self.root / "engine/manifest/relation_types.toml"
        with manifest.open("a") as stream:
            stream.write('\n[[set]]\nname = "CONFIGURE_REGRESSION"\nmembers = ["HAS_NAME"]\n')
