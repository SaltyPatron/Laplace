# ADR 0057: Substrate emission discipline — product yes, packaging no (universal dissolve-and-resynthesize principle at the emission boundary)

> **Audit note (per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md)):** the labels "Food principle," "Vampire mode," and "codec" are cute names that stand in for the mechanism (anchor truth 10). The mechanism this ADR governs is: **dissolve any source into typed, sourced, Glicko-2-rated attestations between content-addressed entities; discard the source blob/packaging; re-synthesize fresh output from substrate state.** The GLOSSARY terms are retained below as cross-references to existing project vocabulary, but the mechanism — not the label — is the invariant. "Codec," wherever it appears below, means exactly this dissolve→resynthesize emission, **not** any round-trip-preserving encode/decode of bytes (preservation is explicitly banned per anchor truth 6).
...
- **Product** — typed knowledge content the substrate extracts as entities + physicalities + attestations. The product is the substrate state itself (typed, sourced, Glicko-2-rated facts), not a stored copy of the source. Observed text content can be reconstructed from its CONTENT physicality trajectory per [ADR 0012 mantissa-packing](0012-mantissa-packing-format.md), and typed-knowledge edges from the attestation rows — but reconstruction-of-observed-bytes is a *property of the trajectory*, not a preservation goal. (Bit-perfect preservation is worthless per [SUBSTRATE-FOUNDATION truth 6](../SUBSTRATE-FOUNDATION.md): it only returns the file you already had.)
...
| **AI models (per [ADR 0056](0056-weight-tensor-etl-as-arena-matchup-observation.md) streaming ETL; GLOSSARY "Vampire mode")** | Recipe entity content (config + tokenizer + architecture metadata as text/JSON); tokenizer vocab as Text entities; significant weight cells emitted as Glicko-2 matchup observations accumulating into consensus ratings on typed mechanical-role attestations between substrate entities per [ADR 0056](0056-weight-tensor-etl-as-arena-matchup-observation.md) (anchor truths 1-2) | safetensors header byte layout, shard file structure, **the weight blob itself** (never stored; only emergent consensus survives), pickle bytestream, ZIP container offsets |
...
### What "reconstructable" means (and why bit-perfect is not the goal)

Bit-perfect preservation is worthless per [SUBSTRATE-FOUNDATION truth 6](../SUBSTRATE-FOUNDATION.md): it only hands back the file you already had. The point below is *not* "the substrate is a faithful archive" — it is that text content is grounded compositional entities (codepoints up the Merkle ladder per truth 7), so the **observed** content is recoverable as a property of the trajectory, while source *packaging* is gone for good.

- **Observed text content is recoverable from its trajectory.** The text "the quick brown fox" reconstructs from its CONTENT physicality trajectory down the Merkle strata to T0 codepoints ([ADR 0047](0047-text-decomposer-pure-primitive.md) — no NFC rewrite at ingest), emitting the **observed** UTF-8 bytes. This is a consequence of codepoint-grounded composition, not a preservation feature.
- **Source packaging is NOT recoverable, by design.** Reconstructing WordNet's exact `data.noun` byte layout from substrate state is not a substrate capability + not a substrate goal. The substrate could emit a *synthesized* WordNet-style file (using the substrate's accumulated synset / lemma / relation knowledge to populate WordNet's data-file format), but the emitted file would be a fresh synthesis — cross-source-enriched, possibly differently-ordered, with the substrate's accumulated knowledge from all sources that touched the same synsets/lemmas. NOT a byte-identical copy of the original `data.noun`.
- **AI models extend the same pattern (per [SUBSTRATE-FOUNDATION truths 1-2, 6](../SUBSTRATE-FOUNDATION.md)).** Weight bytes are never stored and never preserved: model ingestion is a streaming O(params) ETL where each significant weight cell is emitted as a Glicko-2 matchup *observation*, accumulating into consensus ratings (truths 1-2). The substrate emits *synthesized* model packages per recipe; the emitted weights are materialized from the consensus of substrate attestations under the recipe's `knowledge_scope` per [ADR 0009 recipe extraction](0009-recipe-extraction-and-overrides.md) + sparse-by-construction emission per [R4](../../RULES.md). The emitted package is loadable by external runtimes (llama.cpp, etc. — per [GLOSSARY Round-trip](../../GLOSSARY.md)) and verified by **behavioral alignment**, not byte-identity to any source. (Note: the recipe is a fillable mold — the *same* dissolve→resynthesize machinery fills the source's own mold (round-trip) or any other shape (retarget) per truth 6. The exact "pour facts into the mold" synthesis algorithm at frontier scale is **OPEN per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md)**.)
...
Per-source round-trip tests are emission-side verifications of *behavioral / typed-knowledge fidelity* of the dissolve→resynthesize cycle, not preservation tests. ("Codec" is avoided here per [SUBSTRATE-FOUNDATION truth 10](../SUBSTRATE-FOUNDATION.md): it implies round-trip byte preservation, which is banned.)
...
- **Per-source round-trip tests verify behavioral / typed-knowledge alignment, not byte equality.** Fidelity tests follow the behavioral framing per [ADR 0037](0037-layered-seed-ingestion-and-model-codec-fidelity.md) (the "codec" label in that ADR's title is the banned cute name per [SUBSTRATE-FOUNDATION truth 10](../SUBSTRATE-FOUNDATION.md); the mechanism is dissolve→resynthesize). No test should assert `cmp original_source_file emitted_source_file → identical`.
...
- **AI model emissions inherit this discipline.** The model case (GLOSSARY "Vampire mode") is a special case of the universal dissolve→resynthesize pattern ("Food principle"). The substrate's emission discipline for models (weights → matchup observations → consensus ratings → sparse-by-construction recipe-driven synthesis per anchor truths 1-2, 6) follows directly from the universal pattern.
...
- **Bit-perfect round-trip tests** (assert emitted source files are byte-identical to original). Rejected — bit-perfect preservation is worthless per [SUBSTRATE-FOUNDATION truth 6](../SUBSTRATE-FOUNDATION.md); it would require preserving packaging + would prevent cross-source enrichment from naturally affecting emission. The behavioral-fidelity framing per ADR 0037 is the correct shape.
