...
| Early result reduction and honest top-k | Partial | `cluster`, `bubble_up`, `surface_sample`, angular KNN, global top relations, band facts, evidence receipts, and salient facts no longer hide guessed candidate multipliers. Rendering has a total hash fallback, so exact ranked heads cannot underfill; read APIs preserve an explicit zero. Remaining fixed limits and transport/operator caps still require classification. |
