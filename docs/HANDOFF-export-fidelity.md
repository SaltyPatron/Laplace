...
2. Deposit TinyLlama: `ingest model D:\Models\hub\models--TinyLlama--TinyLlama-1.1B-Chat-v1.0\snapshots\fe8a4ea1ffedaf415f4da2f062534de366a451e6`. Expect ~1.1B relations now (per-layer), not 153M. **B1 bulk-fresh apply is committed** (NpgsqlSubstrateWriter `bulkFreshSource`, set true by IngestModelAsync) — skips the attestation existence check. If apply is still the bottleneck, B2 (drop attestations secondary indexes before the role stream, rebuild after) is the next lever — NOT yet built.
