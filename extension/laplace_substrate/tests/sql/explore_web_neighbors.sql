...
-- Composite array scans retain pair election and duplicate input positions,
-- with unrelated families sharing both endpoint indexes in DEFAULT.
DO $$
DECLARE
    s bytea := public.laplace_hash128_blake3('test/composite-scan/subject');
    o bytea := public.laplace_hash128_blake3('test/composite-scan/object');
    a bytea := public.laplace_hash128_blake3('test/composite-scan/type-a');
    b bytea := public.laplace_hash128_blake3('test/composite-scan/type-b');
    noise bytea := public.laplace_hash128_blake3('test/composite-scan/noise-type');
    hits bigint;
BEGIN
    INSERT INTO laplace.consensus
        (id,subject_id,type_id,object_id,rating,rd,volatility,witness_count,last_observed_at)
    SELECT laplace.consensus_id(s,t,o),s,t,o,1800000000000,30000000000,60000000,1,now()
    FROM unnest(ARRAY[a,b]) t;
    INSERT INTO laplace.consensus
        (id,subject_id,type_id,object_id,rating,rd,volatility,witness_count,last_observed_at)
    SELECT laplace.consensus_id(src,noise,dst),src,noise,dst,
           1900000000000,30000000000,60000000,1,now()
    FROM (
        SELECT CASE WHEN i%2=0 THEN s ELSE n END AS src,
               CASE WHEN i%2=0 THEN n ELSE s END AS dst
        FROM generate_series(1,1000) i
        CROSS JOIN LATERAL (
            SELECT public.laplace_hash128_blake3(convert_to('test/composite-scan/noise/' || i, 'UTF8')) AS n
        ) x
    ) cells;

    SELECT count(*) INTO hits
    FROM consensus.explore_web_neighbors(ARRAY[s,s,o],ARRAY[a,b,a],8,false);
    IF hits <> 3 THEN
        RAISE EXCEPTION 'FAIL: composite array scan lost pair election or duplicate frontier positions: %', hits;
    END IF;

    -- Steering intersects both endpoint sets. Exercise the two-key prefix
    -- without a family restriction and the complete three-key prefix with one.
    SELECT count(*) INTO hits
    FROM generation.steer_candidates(ARRAY[o,o],ARRAY[s,s]) q
    WHERE q.candidate=o AND q.edges=2 AND q.covered=1;
    IF hits <> 1 THEN
        RAISE EXCEPTION 'FAIL: untyped endpoint intersection admitted unrelated cells';
    END IF;
    SELECT count(*) INTO hits
    FROM generation.steer_candidates(ARRAY[o,o],ARRAY[s,s],ARRAY[a,b,a]) q
    WHERE q.candidate=o AND q.edges=2 AND q.covered=1;
    IF hits <> 1 THEN
...
-- A shared metadata target is not a semantic path to its other members.
-- Reverse symmetric evidence remains reachable, including at a bounded cutoff.
DO $$
DECLARE
    seed bytea := public.laplace_hash128_blake3('test/directed/seed');
    language bytea := public.laplace_hash128_blake3('test/directed/language');
    unrelated bytea := public.laplace_hash128_blake3('test/directed/unrelated');
    synonym bytea := public.laplace_hash128_blake3('test/directed/synonym');
    directed bytea := laplace.relation_type_id('HAS_LANGUAGE');
    sym_type bytea := laplace.relation_type_id('IS_SYNONYM_OF');
    hits bigint;
BEGIN
    INSERT INTO laplace.consensus
        (id,subject_id,type_id,object_id,rating,rd,volatility,witness_count,last_observed_at)
    VALUES
        (laplace.consensus_id(seed,directed,language),seed,directed,language,
         1800000000000,30000000000,60000000,1,now()),
        (laplace.consensus_id(unrelated,directed,language),unrelated,directed,language,
         9900000000000,30000000000,60000000,1,now()),
        (laplace.consensus_id(synonym,sym_type,language),synonym,sym_type,language,
         1600000000000,30000000000,60000000,1,now());

    SELECT count(*) INTO hits
    FROM consensus.explore_web(ARRAY[seed],2,1,NULL) w
    WHERE w.source_id=unrelated AND w.object_id=language;
    IF hits <> 1 THEN
        RAISE EXCEPTION 'FAIL: browse no longer traverses its inbound metadata edge';
    END IF;
    SELECT count(*) INTO hits
    FROM generation.forward_route_trace_ids(ARRAY[seed],2,1,NULL) w
    WHERE w.source_id=unrelated;
    IF hits <> 0 THEN
        RAISE EXCEPTION 'FAIL: forward route inverted a directed metadata relation';
    END IF;
    SELECT count(*) INTO hits
    FROM generation.forward_route_trace_ids(ARRAY[seed],2,1,NULL) w
    WHERE w.source_id=synonym AND w.object_id=language AND w.hop=2;
    IF hits <> 1 THEN
        RAISE EXCEPTION 'FAIL: reverse symmetric evidence lost its fanout slot';
    END IF;
END $$;
