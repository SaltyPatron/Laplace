...
Initial runner setup (ADR 0014) used Anthony's interactive OS user (`ahart`) for the GitHub Actions runner. Problems:

- Conflates Anthony's interactive identity with CI's automated identity.
- Sudo operations in CI require Anthony's password — blocking automation.
...
- A compromised runner would have Anthony's identity, not a sandboxed CI role.
...
- CI runs as a system account with bounded permissions — proper separation from interactive Anthony identity.
- Compromised runner can install PG extensions (bounded scope), not anything Anthony's sudo can do.
- Anthony's `ahart` PG role no longer needs superuser privileges (ADR cleanup step).
