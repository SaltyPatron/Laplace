...
- **Product is bit-perfect emittable at the canonical-content level.** The text "the quick brown fox" reconstructs from its CONTENT physicality trajectory through T3 sentence → T2 words → T1 graphemes → T0 codepoints, emitting NFC-normalized UTF-8 bytes exactly. Walking the trajectory produces a bit-identical reconstruction.
