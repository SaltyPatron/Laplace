...
Without this ADR, every per-source decomposer that touches a binary container (`ModelDecomposer` composite per [ADR 0043](0043-composite-decomposer-architecture.md) hits this for safetensors / PyTorch / ONNX / TF SavedModel / YOLO; future image / audio decomposers hit it for PNG / JPEG / FLAC / MP4; the same universal posture generalizes to any container — AI models, document corpora, image archives, video collections, audio libraries, code repositories, scientific datasets, knowledge graphs, database exports, web archives, ANY structured or semi-structured data ecosystem. The mechanism (not the label): dissolve the artifact to semantic facts and discard the blob — bit-perfect preservation is worthless per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 6 (cf. [GLOSSARY.md](../../GLOSSARY.md)). Without this ADR, each such decomposer would either invent its own defensive-loading shape or import a third-party loader and absorb its semantics + security surface.
...
When a tensor blob is referenced (`torch._utils._rebuild_tensor_v2` opcode pattern with offset metadata), the parser emits `TensorReference { TensorName, Dtype, Shape, OffsetBytes, LengthBytes }`. Downstream hands the raw tensor bytes to `TensorDtypeDecoder<FP16/BF16/FP32/etc.>` per [ADR 0043](0043-composite-decomposer-architecture.md), which decodes to canonical numerical form. Downstream of dtype decode, the tensor cells are streamed and emitted as Glicko-2 matchup observations (O(params) ETL, never a recompute) per [docs/SUBSTRATE-FOUNDATION.md](../SUBSTRATE-FOUNDATION.md) truth 1. **How interior `d×d` tensor axes (q/k/v/o/gate/up/down) resolve to token entities without re-running the GEMM is OPEN per docs/SUBSTRATE-FOUNDATION.md (OPEN QUESTIONS) and is not settled by this ADR** — `embed_tokens`/`lm_head` are directly token-anchored; the interior resolution must be pinned with Anthony. The container parser's job ends at producing `TensorReference`; the resolution mechanism is downstream and out of scope here.
...
- **One universal posture toward containers**: never load, always statically dissect, then dissolve to semantic facts and discard the blob (per docs/SUBSTRATE-FOUNDATION.md truth 6). The same posture covers AI-model containers and every other container format the substrate ever ingests.
...
- **Treat PyTorch / ONNX / TF as "opaque containers" and store the bytes as substrate content entities, hand them to a downstream tool for query.** Rejected — the substrate does not preserve packaging or weight bytes; bit-perfect preservation only returns the file you already had (docs/SUBSTRATE-FOUNDATION.md truth 6). Static parse + exploded view + dissolve-to-facts is the correct posture.
