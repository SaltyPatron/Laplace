...
DECLARE
    ...
    hp       bytea := laplace.relation_type_id('HAS_PLAYER');
    qw       bytea := set_bit(decode(repeat('00', 32), 'hex'), laplace.qualifier_bit('side', 'white'), 1);
    qb       bytea := set_bit(decode(repeat('00', 32), 'hex'), laplace.qualifier_bit('side', 'black'), 1);
    ...
    src2     bytea := public.laplace_hash128_blake3('test/chess/source-duplicate');
...
BEGIN
    ...
    INSERT INTO laplace.attestations
        (id, subject_id, type_id, object_id, source_id, context_id,
         ...
         sum_score_fp1e9, opponent_rd_fp1e9, qualifier_mask)
    VALUES
        (public.laplace_hash128_blake3('test/chess/ev_strong'), pos, mv, n_strong, src, g_white, 2, now(), 3, 3000000000, 30000000000, NULL),
        -- A second source witnessed the same transition in the same playing. Repertoire
        -- counts playings, so this corroborates that occurrence rather than inventing a game.
        (public.laplace_hash128_blake3('test/chess/ev_strong_source2'), pos, mv, n_strong, src2, g_white, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('test/chess/ev_mid'),    pos, mv, n_mid,    src, g_black, 0, now(), 1, 0, 30000000000, NULL),
        (public.laplace_hash128_blake3('test/chess/pl1'), g_white, plns, l_white, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('test/chess/pl2'), g_black, plns, l_black, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('test/chess/hw1'), l_white, hp, player, src, g_white, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('test/chess/hb1'), l_white, hp, rival,  src, g_white, 2, now(), 1, 1000000000, 30000000000, qb),
        (public.laplace_hash128_blake3('test/chess/hw2'), l_black, hp, rival,  src, g_black, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('test/chess/hb2'), l_black, hp, player, src, g_black, 2, now(), 1, 1000000000, 30000000000, qb),
        (public.laplace_hash128_blake3('test/chess/hr1'), l_white, hr, r_white, src, g_white, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('test/chess/hr2'), l_black, hr, r_white, src, g_black, 2, now(), 1, 1000000000, 30000000000, NULL);
    ...
    -- chess_player_moves: exact successor comes from MOVE evidence indexed by position;
    -- neither line has a physicality, proving this read never unpacks the game trajectory.
    -- Score still comes from the result witnessed under that same playing context.
    ...
    SELECT count(*) INTO n FROM consensus.by_ids(ARRAY[c_strong, c_mid, c_thin], hp);
    IF n <> 0 THEN RAISE EXCEPTION 'FAIL: typed consensus.by_ids(HAS_PLAYER) got % rows', n; END IF;
...
DECLARE
    ...
    -- #1258 retired stored PGN, so nothing deposits HAS_MOVETEXT any more. The
    -- fixture still deposits one below on purpose: it stands in for a row ingested
    -- BEFORE that change, and the chess_game block pins that such a row cannot
    -- bring the movetext column back onto the read surface.
    ...
    profile_only bytea := chess.player_id('Profile, Only');
...
BEGIN
    ...
    INSERT INTO laplace.attestations
        (id, subject_id, type_id, object_id, source_id, context_id,
         ...
         sum_score_fp1e9, opponent_rd_fp1e9, qualifier_mask)
    VALUES
        (public.laplace_hash128_blake3('t2/e1pl'), e1, plns, ln_a, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e2pl'), e2, plns, ln_b, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e3pl'), e3, plns, ln_c, src, NULL, 1, now(), 1, 500000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e4pl'), e4, plns, ln_a, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e_no_traj_pl'), e_no_traj, plns, ln_no_traj, src, NULL, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1w'), ln_a, hp, tal,  src, e1, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('t2/e1b'), ln_a, hp, botv, src, e1, 2, now(), 1, 1000000000, 30000000000, qb),
        (public.laplace_hash128_blake3('t2/e1r'), ln_a, hr, r_white, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1d'), ln_a, od, d1, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1e'), ln_a, hev, en, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1c'), ln_a, heco, ec, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1m'), ln_a, hmt, mt, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e2w'), ln_b, hp, botv, src, e2, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('t2/e2b'), ln_b, hp, tal,  src, e2, 2, now(), 1, 1000000000, 30000000000, qb),
        (public.laplace_hash128_blake3('t2/e2r'), ln_b, hr, r_white, src, e2, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e2d'), ln_b, od, d2, src, e2, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e3w'), ln_c, hp, tal,  src, e3, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('t2/e3b'), ln_c, hp, spas, src, e3, 2, now(), 1, 1000000000, 30000000000, qb),
        (public.laplace_hash128_blake3('t2/e3r'), ln_c, hr, r_draw, src, e3, 1, now(), 1, 500000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e3d'), ln_c, od, d3, src, e3, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e4w'), ln_a, hp, tal,  src, e4, 2, now(), 1, 1000000000, 30000000000, qw),
        (public.laplace_hash128_blake3('t2/e4b'), ln_a, hp, botv, src, e4, 2, now(), 1, 1000000000, 30000000000, qb),
        ...
        (public.laplace_hash128_blake3('t2/mv_e1'), pos2, mv, mv_w, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/mv_e2'), pos2, mv, mv_b, src, e2, 0, now(), 1, 0, 30000000000, NULL),
        -- the players' own aggregating lane (AppendPlayerResult), ctx = the playing
        (public.laplace_hash128_blake3('t2/e1ot'), tal,  outc, cres, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e1ob'), botv, outc, cres, src, e1, 0, now(), 1, 0, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e2ob'), botv, outc, cres, src, e2, 2, now(), 1, 1000000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e2ot'), tal,  outc, cres, src, e2, 0, now(), 1, 0, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e3ot'), tal,  outc, cres, src, e3, 1, now(), 1, 500000000, 30000000000, NULL),
        (public.laplace_hash128_blake3('t2/e3os'), spas, outc, cres, src, e3, 1, now(), 1, 500000000, 30000000000, NULL),
        -- an Elo tag whose surface cannot be rendered back to digits
        (public.laplace_hash128_blake3('t2/rat'), tal, hrat, elo, src, e1, 2, now(), 1, 1000000000, 30000000000, NULL);
    ...
    -- sides followable back to their careers, every header realized from its surface
    ...
    SELECT event INTO txt FROM chess.game(e1);
    IF txt IS DISTINCT FROM 'e' THEN RAISE EXCEPTION 'FAIL: e1 event should realize to e, got %', txt; END IF;
    SELECT eco INTO txt FROM chess.game(e1);
    IF txt IS DISTINCT FROM 'C' THEN RAISE EXCEPTION 'FAIL: e1 eco should realize to C, got %', txt; END IF;
    -- #1258: PGN is not a substrate content object. A playing's mainline IS its line's
    -- typed move trajectory, and SAN/PGN is generated by replay at the read edge, so
    -- chess.game must not advertise a movetext column at all — a column that could only
    -- ever return NULL promises a value the substrate cannot produce. The fixture still
    -- deposits a legacy (ln_a, HAS_MOVETEXT, 'm') testimony in e1's context, so this
    -- also pins that pre-#1258 rows cannot resurface the column.
    IF EXISTS (SELECT 1 FROM pg_proc p
               JOIN pg_namespace ns ON ns.oid = p.pronamespace
               WHERE ns.nspname = 'chess' AND p.proname = 'game'
                 AND 'movetext' = ANY(p.proargnames)) THEN
        RAISE EXCEPTION 'FAIL: chess.game still advertises a movetext column';
    END IF;
    -- and the mainline that replaced it is reachable: e1 PLAYS_LINE ln_a, whose Content
    -- physicality carries the ordered typed moves the read edge replays into SAN.
    SELECT count(*) INTO n
    FROM laplace.attestations a
    JOIN laplace.physicalities ph ON ph.entity_id = a.object_id AND ph.type = 1
    WHERE a.subject_id = e1 AND a.type_id = plns AND ph.n_constituents > 0;
    IF n <> 1 THEN
        RAISE EXCEPTION 'FAIL: e1 mainline should be its line''s typed move trajectory, got % rows', n;
    END IF;
    ...
    -- chess_player_moves across the re-key: the exact successor comes from indexed MOVE
    -- evidence, and colour/result testimony is joined through the playing context.
    -- e4 shares ln_a but has no result, so it remains unscored.
    ...
    SELECT player_id INTO got FROM chess.ranked(1, 0, 'games', 'asc');
    IF got <> spas THEN RAISE EXCEPTION 'FAIL: chess_ranked games ASC should put the one-game player first'; END IF;
    ...
    VALUES (public.laplace_hash128_blake3('t2/name/tal_phys'), tname, 1,
            'SRID=0;POINT ZM (0 0 0 0)'::geometry, decode(repeat('00', 16), 'hex'),
            public.laplace_trajectory_build(ARRAY[cap_t, en]), 2, now());
    INSERT INTO laplace.consensus
        (id, subject_id, type_id, object_id, rating, rd, volatility, witness_count, last_observed_at)
    VALUES
    ...
    SELECT p.games, p.rating, p.rd, p.eff_mu INTO n, ra, rdv, mu
      FROM chess.player_search_candidates(ARRAY['Te'], 10) p
     WHERE p.player_id = profile_only;
    IF n <> 0 OR ra <> 1500.0 OR rdv <> 350.0 OR mu <> 800.0 THEN
        RAISE EXCEPTION 'FAIL: profile-only player was hidden or assigned invented career evidence: %/%/%/%',
            n, ra, rdv, mu;
    END IF;

    SELECT count(*) INTO n
      FROM chess.players_by_initial(ARRAY['T'], 10, 0, 'strength', 'desc') p
     WHERE p.player_id = profile_only;
    IF n <> 1 THEN
        RAISE EXCEPTION 'FAIL: profile-only player was hidden from initial browse';
    END IF;

    SELECT p.games, p.rating, p.rd, p.eff_mu INTO n, ra, rdv, mu
      FROM chess.ranked(10) p WHERE p.player_id = profile_only;
    IF n <> 0 OR ra <> 1500.0 OR rdv <> 350.0 OR mu <> 800.0 THEN
        RAISE EXCEPTION 'FAIL: profile-only player was hidden from master list or assigned invented standing: %/%/%/%',
            n, ra, rdv, mu;
    END IF;
    ...
    IF EXISTS(SELECT 1 FROM chess.ranked(100,0,'games','asc')
        ...
        RAISE EXCEPTION 'FAIL: player type without evidence became a standing';
    END IF;

    IF to_regprocedure('chess.game_plies(bytea)') IS NOT NULL THEN
        RAISE EXCEPTION 'FAIL: retired per-ply SQL board projection is still installed';
    END IF;
