...
public static class ChessGraph
{
        ...
        Hash128 src, long nowUs, PhysicalityType type = PhysicalityType.Content)
    {
        ...
        // Preserve the established chess placement recipe for both Content and
        // Projection. The generic centroid/Karcher reconciliation is a separate
        // declared migration; trajectory identity repair does not choose it.
        double[] centroid = Math4d.KarcherMean(coords);
    ...
    /// <summary>
    /// Record player results and pairing evidence in Laplace consensus. Source Elo
    /// belongs to HAS_RATING testimony; it is not a Laplace opponent standing.
    /// These evidence cells use the same neutral comparison and witness RD as
    /// other categorical testimony. Each game retains its own provenance context.
    /// </summary>
        ...
        double witnessWeight, Hash128 src, Hash128 gameId)
    {
        ...
        b.AddAttestation(Outcome(
            player, games: 1, sum, witnessWeight, src, gameId));
        if (opponent is { } opp)
            b.AddAttestation(NativeAttestation.Aggregated(
                subject: player,
