...
Per [ADR 0021](../docs/adr/0021-dbup-for-migrations.md) (DbUp + Npgsql) and [ADR 0023](../docs/adr/0023-extension-owns-schema-dbup-orchestrates.md) (extension owns schema; DbUp orchestrates), this directory holds **extension-lifecycle migrations** — NOT substrate schema. The substrate's three core tables (`entities`, `physicalities`, `attestations`), custom types, GIST opclasses, SRFs, and aggregates live in `extension/laplace--*.sql`, owned by the `laplace` extension itself.

## Scope

DbUp migrations in `db/migrations/` do exactly four things:

1. **Ensure prerequisite extensions** — `CREATE EXTENSION IF NOT EXISTS postgis; CREATE EXTENSION IF NOT EXISTS laplace;`
...
4. **Trigger extension upgrades** — `ALTER EXTENSION laplace UPDATE TO 'A.B.C';` (lands in a new migration file when the extension's `default_version` bumps)

**Substrate schema changes do NOT go here.** They go in `extension/laplace--<from>--<to>.sql` upgrade scripts.

If you find yourself writing `CREATE TABLE laplace.entities (...)` in a `db/migrations/` file, **stop** — that table belongs to the extension. Move it to the extension's SQL.
...
- **Don't create substrate tables here.** They belong in the extension's SQL.
