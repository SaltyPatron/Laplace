...
public sealed class ContentWitnessContainmentTests
{
    ...
    public void PresentBitmap_StagesNothing(string s)
    {
        ...
        // Present content already is its entity and its one composition physicality.
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task StoredRootCoversItsSubtree(bool cached)
    {
        ...
        // The trunk is asked once; everything under a present trunk is present.
        for (uint i = 0; i < tree!.NodeCount; i++)
            Assert.True(BitmapBits.IsSet(bitmap, (int)i));
        Assert.DoesNotContain(reader.Probed, id => id != root);

        using var covered = IntentStage.New(256);
        Assert.True(covered.EmitContentTree(tree, Src, bitmap, out var coveredRoot));
        Assert.Equal(root, coveredRoot);
        Assert.Equal(0, covered.EntityCount);
        Assert.Equal(0, covered.PhysicalityCount);
    }
