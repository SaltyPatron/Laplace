...
| DN-08A | Traversal weights and work bounds come from substrate state or callers | Delivered for A* and edge strength; broader scoring audit open | #1236 derives strength from Glicko-2 expected score, turns path likelihood into additive negative-log cost, removes the engine's 256-neighbor truncation and implicit depth defaults, and uses geometry only as an equal-cost ordering hint so it cannot corrupt the least-cost result. | [#588](https://github.com/SaltyPatron/Laplace/issues/588), [#1135](https://github.com/SaltyPatron/Laplace/issues/1135): remove remaining fitted scoring constants and classify every public work bound. |
