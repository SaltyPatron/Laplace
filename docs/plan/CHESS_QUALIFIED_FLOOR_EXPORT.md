...
The existing `scripts/export-qualified-chess-floors.py` driver prepares an authenticated recorded-corpus input for the normal cache build. Invoke it through the retained operator on `verify/chess-floor-serving-controls-20260916`, using the existing `.github/workflows/chess-floor-serving-controls.yml` owner and its scheduled host lock. The separate export workflow has been removed. Export does not install files, restart services, mutate the database, or merge a pull request.
