...
static std::vector<size_t> segment_all(const std::vector<uint32_t>& cps) {
    ...
    size_t pos = 0;
    while (true) {
        size_t nxt = laplace_word_break_next(cps.data(), cps.size(), pos);
        b.push_back(nxt);
...
TEST(LaplaceCoreWordBreak, UAX29ConformancePerLine) {
    ...
    for (const auto& tc : cases) {
        ...
        if (got == tc.breaks) ++pass;
        else { ++fail; if (failed_lines.size() < 20) failed_lines.push_back(tc.line_number); }
    }
    std::cerr << "[WordBreak conformance UCD " << LAPLACE_UCD_PATH_FOR_TESTS
