...
Implemented as **one** script: `scripts/bootstrap-laplace-runner.sh`. Idempotent. the developer runs ONCE: `sudo bootstrap-laplace-runner.sh`. Forever-after.
...
- Layer 1 means the agent does the ongoing work — schema migrations, extension upgrades, ingestion — without further manual involvement.
- Layer 2 lets the developer reproduce CI state locally for debug.
