...
BEGIN
    -- consensus carries real FKs to entities (the content swap preserves them;
    -- the old identity swap silently produced an FK-less table, which is the
    -- only reason this fixture ever worked without these rows).
    INSERT INTO entities (id, tier, type_id, first_observed_by)
    VALUES (s, 0, type_t, NULL), (t, 0, type_t, NULL), (o, 0, type_t, NULL)
    ON CONFLICT DO NOTHING;
    DELETE FROM consensus;
    INSERT INTO consensus (id, subject_id, type_id, object_id,
                           rating, rd, volatility, witness_count, last_observed_at)
    VALUES (consensus_id(s, t, o), s, t, o,
            1500000000000, 350000000000, 60000000, 1, '2026-01-01 00:00:00+00');

    IF to_regclass('consensus_next') IS NOT NULL THEN EXECUTE 'DROP TABLE consensus_next'; END IF;
    CREATE TABLE consensus_next (
        id bytea, subject_id bytea, type_id bytea, object_id bytea,
        rating bigint, rd bigint, volatility bigint,
        witness_count bigint, last_observed_at timestamptz);
    ...
    END;
    IF NOT refused THEN
        RAISE EXCEPTION 'FAIL(guard): empty consensus_next swap over populated consensus was not refused';
    END IF;
    IF (SELECT count(*) FROM consensus) <> 1 THEN
    ...
    IF (SELECT count(*) FROM consensus) <> 0 THEN
        RAISE EXCEPTION 'FAIL(guard): override did not permit the empty swap';
    END IF;
