# Inventory (GENERATED — do not edit)

Generated by `python3 scripts/docs-inventory.py` (Windows:
`scripts\win\docs-inventory.cmd`). The git pre-commit hook
(`scripts/githooks/pre-commit`, installed via `scripts/install-githooks.sh`
/ `scripts/agent-worktree.sh`) refreshes and stages this file automatically.
CI only `--check`s drift as a backstop. Prose docs cite this file instead of
embedding counts.

## Solution — 12 projects + 9 test projects (`app/Laplace.slnx`)

- `Laplace.Agents`
- `Laplace.Chess`
- `Laplace.Chess.Uci`
- `Laplace.Cli`
- `Laplace.Core`
- `Laplace.Decomposers`
- `Laplace.Endpoints.Lichess`
- `Laplace.Endpoints.Mcp`
- `Laplace.Endpoints.OpenAICompat`
- `Laplace.Migrations`
- `Laplace.Ops`
- `Laplace.Substrate`

Test projects:

- `Laplace.Agents.Tests`
- `Laplace.Chess.Tests`
- `Laplace.Cli.Tests`
- `Laplace.Core.Tests`
- `Laplace.Decomposers.Tests`
- `Laplace.Endpoints.Lichess.Tests`
- `Laplace.Endpoints.Mcp.Tests`
- `Laplace.Endpoints.OpenAICompat.Tests`
- `Laplace.Substrate.Tests`

## Decomposers — 31 in `Laplace.Decomposers`, 13 in `Laplace.Chess`

- `AgentTraceDecomposer`
- `Atomic2020Decomposer`
- `CILIDecomposer`
- `CodeDecomposer`
- `ConceptNetDecomposer`
- `FrameNetDecomposer`
- `FrameVideoDecomposer`
- `ISODecomposer`
- `MapNetDecomposer`
- `ModelDecomposer`
- `OMWDecomposer`
- `OpenSubtitlesDecomposer`
- `OperationalDecomposer`
- `ParquetDecomposer`
- `PropBankDecomposer`
- `RecipeDecomposer`
- `RepoDecomposer`
- `RgbaImageDecomposer`
- `SemLinkDecomposer`
- `StackDecomposer`
- `TabularDecomposer`
- `TatoebaDecomposer`
- `TinyCodesDecomposer`
- `TrackAudioDecomposer`
- `UDDecomposer`
- `UnicodeDecomposer`
- `VerbNetDecomposer`
- `VerifiedGitRepoDecomposer`
- `WiktionaryDecomposer`
- `WordFrameNetDecomposer`
- `WordNetDecomposer`

Chess:

- `ChessAnalyzeDecomposer`
- `ChessBookDecomposer`
- `ChessMoveOutcomesDecomposer`
- `ChessOpeningMatchDecomposer`
- `ChessOpeningsDecomposer`
- `ChessPgnDecomposer`
- `ChessPlayerContextOutcomesDecomposer`
- `ChessPositionOutcomesDecomposer`
- `ChessStockfishEvalDecomposer`
- `ChessSyzygyDecomposer`
- `ChessTacticOutcomesDecomposer`
- `ChessTrajectoryDecomposer`
- `ChessTransitionsDecomposer`

## SQL function families — 29 (`extension/laplace_substrate/sql/functions/`)

- `analysis`
- `cascade`
- `chess`
- `consensus`
- `contrast`
- `converse`
- `corpus`
- `fold`
- `generation`
- `geometry`
- `glicko2`
- `highway`
- `identity`
- `inference`
- `ingest`
- `inspect`
- `lexical`
- `link`
- `model`
- `mu`
- `ops`
- `readback`
- `realize`
- `recall`
- `relation`
- `structural`
- `taxonomy`
- `trajectory`
- `variant`

## Native extension sources — 59 (`extension/laplace_substrate/src/`)

- `astar_path.c`
- `attestation_response.c`
- `band_leaders.c`
- `browse_native.c`
- `chess_learned_moves.c`
- `chess_rating_repair.c`
- `chess_roster.c`
- `chess_syzygy.c`
- `cognition_program.c`
- `consensus_bulk_write.c`
- `consensus_fold_step.c`
- `consensus_neighbors.c`
- `consensus_scan.c`
- `constituents_closure.c`
- `containers_of.c`
- `content_carrier_vertices.c`
- `content_membership_read.c`
- `content_resolve.c`
- `content_text_read.c`
- `content_trajectory_read.c`
- `conversation_session.c`
- `descent_probe.c`
- `display_native.c`
- `entity_mask_write.c`
- `execution_module.c`
- `explore_web.c`
- `fold_route.c`
- `foundry_crawl.c`
- `generate_walk.c`
- `generated_stage_sink.c`
- `geometry_successors.c`
- `grammar_source.c`
- `graph_cascade.c`
- `graph_contrast.c`
- `graph_taxonomy.c`
- `highway_mask.c`
- `identity_scan.c`
- `laplace_substrate.c`
- `lexical_case.c`
- `model_factor.c`
- `observation_read.c`
- `perfcache.c`
- `physicality_descriptor_admission.c`
- `physicality_descriptor_readback.c`
- `prompt_coherence.c`
- `prompt_language.c`
- `prompt_structure.c`
- `query_evidence.c`
- `realize_batch.c`
- `recall.c`
- `recall_route.c`
- `steer_candidates.c`
- `steered_walk.c`
- `task_shape.c`
- `text_root_placements.c`
- `trajectory_continuations.c`
- `trajectory_generate.c`
- `variant_synth.c`
- `word_adjacency.c`

## Relation manifest (`engine/manifest/relation_types.toml`)

- Governed canonical relations: **226** (+23 aliases — aliases map to a canonical and add no highway bits)
- Salience bands: **13** — `mandate`, `definitional`, `taxonomic`, `equivalence`, `partitive`, `causal`, `oppositional`, `associative`, `tensor_calculation`, `lexical_glue`, `scalar_valued`, `standards_structural`, `probationary`

Highway bits are an explicit append-only `bit = N` registry in
`relation_types.toml` (ADR 0001 / GH #551). Codegen validates uniqueness and
range; adding a relation takes the next free bit and does not renumber peers.

## Perfcache blobs referenced in source

- `laplace_chess_position_perfcache.bin`
- `laplace_chess_transition_perfcache.bin`
- `laplace_highway_perfcache.bin`
- `laplace_modality_number_perfcache.bin`
- `laplace_old_perfcache.bin`
- `laplace_t0_perfcache.bin`
