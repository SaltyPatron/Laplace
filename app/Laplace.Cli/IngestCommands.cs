...
internal static class IngestCommands
{
    ...
    // Seed-wide index-free bulk load. The per-decomposer SecondaryIndexPolicy only drops indexes when
    // a table is still empty, so once the floor stage populates physicalities every later decomposer
    // pays LIVE coord-GiST / trajectory-GIN maintenance per novel insert (the dominant cost of a fresh
    // reseed). `indexes suspend` drops every secondary (non-primary, non-unique) index on the bulk
    // substrate tables ONCE up front and records their CREATE defs in a durable table so a separate
    // CLI process (the end of the ladder) can `indexes rebuild` them in a single bulk build. PK/unique
    // indexes are kept — content-addressed ON CONFLICT and the existence probes still work index-free.
    private static readonly string[] DefaultBulkIndexTables =
        { "entities", "physicalities", "attestations", "consensus" };

    private static string[] BulkIndexTables()
    {
        var env = Environment.GetEnvironmentVariable("LAPLACE_BULK_INDEX_TABLES");
        if (string.IsNullOrWhiteSpace(env)) return DefaultBulkIndexTables;
        var list = env.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        return list.Length > 0 ? list : DefaultBulkIndexTables;
    }

    public static async Task<int> IndexesAsync(string[] args)
    {
        if (args.Length == 0)
            return Fail("usage: laplace indexes <suspend|rebuild|status>\n"
                        + "  suspend: drop secondary indexes on the bulk substrate tables (seed-wide index-free load)\n"
                        + "  rebuild: recreate every suspended index in one bulk build, then clear the record\n"
                        + "  status : list indexes currently suspended\n"
                        + "  tables : LAPLACE_BULK_INDEX_TABLES (default: " + string.Join(",", DefaultBulkIndexTables) + ")");

        await using var ds = new NpgsqlDataSourceBuilder(ConnString).Build();
        return args[0].ToLowerInvariant() switch
        {
            "suspend" => await IndexesSuspendAsync(ds),
            "rebuild" => await IndexesRebuildAsync(ds),
            "status"  => await IndexesStatusAsync(ds),
            _ => Fail($"indexes: unknown subcommand '{args[0]}' (expected suspend|rebuild|status)"),
        };
    }

    private static async Task EnsureSuspendStoreAsync(NpgsqlConnection conn, CancellationToken ct = default)
    {
        await using var cmd = conn.CreateCommand();
        cmd.CommandText =
            "CREATE TABLE IF NOT EXISTS laplace.suspended_indexes ("
            + "  def text PRIMARY KEY,"
            + "  suspended_at timestamptz NOT NULL DEFAULT now())";
        await cmd.ExecuteNonQueryAsync(ct);
    }

    private static async Task<int> IndexesSuspendAsync(NpgsqlDataSource ds)
    {
        var tables = BulkIndexTables();
        await using var conn = await ds.OpenConnectionAsync();
        await EnsureSuspendStoreAsync(conn);
        ...
        await using (var q = conn.CreateCommand())
        {
            q.CommandText =
                ...
                + "WHERE n.nspname = 'laplace' AND t.relname = ANY(@tables) "
                + "  AND NOT i.indisprimary AND NOT i.indisunique";
            q.Parameters.AddWithValue("tables", tables);
            await using var r = await q.ExecuteReaderAsync();
            while (await r.ReadAsync())
                defs.Add((r.GetString(0), r.GetString(1)));
        }

        if (defs.Count == 0)
        {
            Console.WriteLine($"indexes suspend: no live secondary indexes on [{string.Join(", ", tables)}] "
                              + "(already suspended, or the schema has none) — nothing to drop");
            return 0;
        }
        ...
        foreach (var (name, def) in defs)
        {
            ...
            {
                ins.CommandText =
                    "INSERT INTO laplace.suspended_indexes (def) VALUES (@def) ON CONFLICT (def) DO NOTHING";
                ins.Parameters.AddWithValue("def", def);
        ...
        Console.WriteLine($"indexes suspend: dropped {dropped} secondary index(es) on "
                          + $"[{string.Join(", ", tables)}]; defs recorded for rebuild");
        return 0;
    }

    private static async Task<int> IndexesRebuildAsync(NpgsqlDataSource ds)
    {
        await using var conn = await ds.OpenConnectionAsync();
        await EnsureSuspendStoreAsync(conn);
        ...
        await using (var q = conn.CreateCommand())
        {
            q.CommandText = "SELECT def FROM laplace.suspended_indexes ORDER BY def";
            await using var r = await q.ExecuteReaderAsync();
            while (await r.ReadAsync())
                defs.Add(r.GetString(0));
        }

        if (defs.Count == 0)
        {
            Console.WriteLine("indexes rebuild: nothing suspended — nothing to rebuild");
            return 0;
        }

        await using (var tune = conn.CreateCommand())
        {
            tune.CommandText =
                "SET maintenance_work_mem = '2GB'; SET max_parallel_maintenance_workers = 4";
            await tune.ExecuteNonQueryAsync();
        }
        ...
        foreach (var def in defs)
        {
            ...
            ixSw.Stop();
            built++;
            Console.WriteLine($"  rebuilt ({built}/{defs.Count}) in {ixSw.Elapsed.TotalSeconds:F1}s: {def}");
        }

        await using (var del = conn.CreateCommand())
        {
            del.CommandText = "DELETE FROM laplace.suspended_indexes";
            await del.ExecuteNonQueryAsync();
        }
        sw.Stop();
        Console.WriteLine($"indexes rebuild: rebuilt {built} secondary index(es) in {sw.Elapsed.TotalSeconds:F1}s");
        return 0;
    }

    private static async Task<int> IndexesStatusAsync(NpgsqlDataSource ds)
    {
        await using var conn = await ds.OpenConnectionAsync();
        await EnsureSuspendStoreAsync(conn);
        await using var q = conn.CreateCommand();
        q.CommandText = "SELECT def, suspended_at FROM laplace.suspended_indexes ORDER BY suspended_at, def";
        await using var r = await q.ExecuteReaderAsync();
        int n = 0;
        while (await r.ReadAsync())
        {
            Console.WriteLine($"  [{r.GetDateTime(1):u}] {r.GetString(0)}");
            n++;
        }
        Console.WriteLine(n == 0 ? "indexes status: none suspended" : $"indexes status: {n} suspended");
        return 0;
    }
