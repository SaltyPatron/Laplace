    ...
    for raw in text.splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            ...
            if inner == "ranks":
                ...
                section = "wordnet"
                current = data["wordnet"]
            elif inner == "wiktionary":
                section = "wiktionary"
                current = data["wiktionary"]
            else:
                section = inner
                current = None
            continue
...
""",
    ...
    lines.append("}")
    lines.append("")
    lines.append("static const char* resolve_wordnet(const char* tag) {")
    lines.append("    if (!tag || !tag[0] || tag[1]) return NULL;")
    lines.append("    switch (tag[0]) {")
    for k, v in sorted(wn.items()):
        if len(k) == 1:
            cases = k if k != "s" else "'a'"
            if k == "a":
                lines.append("        case 'a': case 's': return \"ADJ\";")
            elif k == "s":
                continue
            else:
                lines.append(f"        case '{k}': return \"{v}\";")
    lines.append("        default: return NULL;")
    lines.append("    }")
    lines.append("}")
    lines.append("")
    lines.append("typedef struct { const char* key; const char* canon; } wiki_map_t;")
    lines.append("static const wiki_map_t k_wiki[] = {")
    for k in sorted(wiki, key=str.lower):
        lines.append(f'    {{ "{k}", "{wiki[k]}" }},')
    lines.append("};")
    lines.append("")
    lines.append("static const char* resolve_wiktionary(const char* tag) {")
    lines.append("    for (size_t i = 0; i < sizeof(k_wiki)/sizeof(k_wiki[0]); ++i) {")
    lines.append("        if (str_ieq(k_wiki[i].key, tag)) return k_wiki[i].canon;")
    lines.append("    }")
    ...
    lines.append("        case LAPLACE_POS_TAGSET_UPOS:       canon = resolve_upos_canonical(tag); break;")
    lines.append("        case LAPLACE_POS_TAGSET_WORDNET:    canon = resolve_wordnet(tag); break;")
    lines.append("        case LAPLACE_POS_TAGSET_WIKTIONARY: canon = resolve_wiktionary(tag); break;")
    ...
    lines.append("    }")
    lines.append("    const char* ns = tagset == LAPLACE_POS_TAGSET_UPOS ? \"upos\"")
    lines.append("                   : tagset == LAPLACE_POS_TAGSET_WORDNET ? \"wordnet\" : \"wiktionary\";")
