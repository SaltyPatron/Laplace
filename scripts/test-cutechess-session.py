...
class CuteChessBootstrapTests(unittest.TestCase):
    ...
    def test_selected_account_is_validated_by_identity_not_username(self):
        class AccountSelected(Exception):
            pass

        for name, uid in (("laplace-runner", 994), ("operator", 1000)):
            with self.subTest(name=name):
                account = mock.Mock(pw_uid=uid)
                # Stop at the first OS metadata read, before any host changes.
                with mock.patch.object(bootstrap.os, "geteuid", return_value=0), \
                     mock.patch.object(bootstrap.pwd, "getpwnam", return_value=account) as lookup, \
                     mock.patch.object(bootstrap.Path, "read_text", side_effect=AccountSelected):
                    with self.assertRaises(AccountSelected):
                        bootstrap.provision(ROOT, name, True)
                lookup.assert_called_once_with(name)

    def test_account_selection_keeps_root_and_missing_identity_refusals(self):
        with mock.patch.object(bootstrap.os, "geteuid", return_value=0), \
             mock.patch.object(bootstrap.Path, "read_text") as read:
            with mock.patch.object(bootstrap.pwd, "getpwnam",
                                   return_value=mock.Mock(pw_uid=0)) as lookup:
                with self.assertRaisesRegex(RuntimeError, "non-root"):
                    bootstrap.provision(ROOT, "root-alias", True)
                lookup.assert_called_once_with("root-alias")
            with mock.patch.object(bootstrap.pwd, "getpwnam") as lookup:
                with self.assertRaisesRegex(RuntimeError, "explicit existing"):
                    bootstrap.provision(ROOT, "", True)
                lookup.assert_not_called()
            with mock.patch.object(bootstrap.pwd, "getpwnam",
                                   side_effect=KeyError("missing account")) as lookup:
                with self.assertRaises(KeyError):
                    bootstrap.provision(ROOT, "missing-account", True)
                lookup.assert_called_once_with("missing-account")
