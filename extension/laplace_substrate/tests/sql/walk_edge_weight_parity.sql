...
SELECT glicko2_neutral_mu() = 1500000000000        AS neutral_matches_engine,
       foundry_witness_sat(4) = 0.5                AS halfmax_is_4,
       foundry_witness_sat(0) = 0.0                AS zero_witness_zero_weight,
       foundry_rd_kappa() = 1.0                    AS kappa_is_1;
...
-- eff_mu >= neutral.
SELECT walk_edge_weight(1600000000000, 350000000000, 1) > 0   AS wide_rd_win_stays_walkable,
       walk_edge_weight(1400000000000,  30000000000, 50) < 0  AS refutation_is_negative,
       walk_edge_weight(1500000000000,  80000000000, 10) = 0  AS neutral_is_zero,
       walk_edge_weight(1600000000000,  30000000000, 50)
         > walk_edge_weight(1600000000000, 350000000000, 50)  AS rd_discounts,
       walk_edge_weight(1600000000000,  30000000000, 50)
         > walk_edge_weight(1600000000000,  30000000000, 1)   AS witnesses_saturate;
...
SELECT walk_edge_weight(1600000000000, 30000000000, 50)
       = walk_edge_weight(1600000000000, 30000000000, 50, foundry_rd_kappa())
...
SELECT walk_edge_weight(1600000000000, 350000000000, 1)::numeric
       <> eff_mu(1600000000000, 350000000000)::numeric AS weight_is_not_eff_mu,
       eff_mu(1600000000000, 350000000000) = 900000000000 AS eff_mu_unchanged;
