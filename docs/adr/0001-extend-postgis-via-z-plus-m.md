...
Use **standard PostGIS `geometry`** with the Z and M flags set (`ST_HasZ AND ST_HasM`), interpreted as 4D (X, Y, Z spatial + M as fourth spatial dim). Index via PostGIS's existing `gist_geometry_ops_nd` N-dimensional GIST opclass.
