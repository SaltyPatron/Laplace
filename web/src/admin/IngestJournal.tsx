...
function Status({ value }: { value: string | null }) {
  return value
    ? <span className={`${styles.badge} ${styles[ingestStatusTone(value)]}`}>{value}</span>
...
function RunWorkspace({ tenant, refreshSignal }: { tenant: string; refreshSignal: number }) {
  ...
  const runsRead = useReadResource({
    key: JSON.stringify(['ingest-runs', tenant, limit]),
    refreshMs: live ? REFRESH_MS : 0,
    read: async (signal) => {
      const result = await ingestRuns(limit, { tenant, signal });
      return captureRows(
        result.rows,
        `Up to ${limit} requested run receipts${result.truncated_at != null ? `; transport truncated at ${result.truncated_at}` : ''}. Older runs may exist.`,
        { operation: 'ops.ingest_runs', requested_limit: limit },
      );
    },
  });
  ...
  async function closeReceipt(run: IngestRun) {
    ...
    try {
      ...
      setConfirming(null);
      await runsRead.refresh();
    } catch (failure) {
      setActionError(
        `${run.source_name}: ${failure instanceof Error ? failure.message : String(failure)}. ` +
        'The outcome may be unknown after a transport failure; refresh the receipt before trying again.',
      );
    } finally {
  ...
  const columns: ResultColumn<IngestRun>[] = [
    ...
    },
    {
      ...
      render: (run) => (
        <div className={styles.runState}>
          ...
          {run.error && (
            <details className={styles.runErr}>
              <summary>Run error</summary>
              <pre className={styles.sig}>{run.error}</pre>
            </details>
          )}
        </div>
      ),
    ...
    {
      key: 'throughput_status',
      label: 'Performance',
      render: (run) => (
        <div className={styles.runPerformance}>
          ...
          <time className={styles.progressPct} dateTime={run.started_at ?? undefined}>
            {run.started_at ? new Date(run.started_at).toLocaleString() : 'Start not recorded'}
          </time>
        </div>
      ),
    },
      ...
      render: (run) => (
        <div className={styles.runActions}>
          <Button
            ...
            onClick={() => setParam('run', expandedRun === run.run_id ? null : run.run_id)}
          >
            {expandedRun === run.run_id ? 'Hide files' : 'File receipts'}
          </Button>
          {OPEN_INGEST_STATES.has(run.status.toLowerCase())
            ? (
              <Button
                variant="ghost"
                disabled={closing}
                onClick={() => { setConfirming(run); setActionError(null); }}
              >
                Close journal…
              </Button>
            )
  ...
  return (
    <>
      <Panel title="Ingestion runs" expandable>
        <div className={styles.toolbar}>
          ...
          <label className={styles.limitLabel}>
            Runs
            <select className={styles.limitSelect} value={limit} onChange={(event) => setLimit(Number(event.target.value))}>
              {[10, 25, 50, 100, 500].map((value) => <option key={value} value={value}>{value}</option>)}
            </select>
          </label>
          <Button variant="ghost" onClick={() => void runsRead.refresh()}>Refresh</Button>
        </div>
        <ReadStatus label="Ingestion runs" resource={runsRead} />
        {open != null && <Muted>{open} open · {runsRead.data?.rows.length ?? 0} returned. Older runs may exist.</Muted>}
        {runsRead.data && (
          <ResultWorkspace
            scopeKey={JSON.stringify(['ingest-runs', tenant])}
            label="Ingestion run receipts"
            snapshot={runsRead.data}
            columns={columns}
            rowLabel={(run) => `${run.source_name} run ${run.run_id}`}
            filterText={filter}
            onFilterTextChange={(value) => setParam('source', value, true)}
            selectable={false}
            compact
          />
        )}
      ...
      {expandedRun && (
        <RunFiles
          key={JSON.stringify([tenant, expandedRun])}
          runId={expandedRun}
          tenant={tenant}
          live={live}
      ...
      <Modal
        ...
        onClose={() => { if (!closingRef.current) setConfirming(null); }}
        title="Close this journal receipt?"
        actions={(
          <>
            <Button variant="ghost" disabled={closing} onClick={() => setConfirming(null)}>Go back</Button>
            <Button loading={closing} onClick={() => confirming && void closeReceipt(confirming)}>Mark journal cancelled</Button>
          </>
        )}
      >
        <p>Source: {confirming?.source_name}. Run: <code>{confirming?.run_id}</code>.</p>
        <p>This changes the shared journal receipt only. It does not stop a CLI process or database backend. Use Activity to inspect and stop active backend work before closing its receipt.</p>
        <Link to="/operator?section=activity">Open Activity</Link>
        <details>
          <summary>Equivalent journal-only SQL</summary>
          <pre className={styles.sig}>{confirming ? `SELECT * FROM ops.ingest_run_close('${confirming.run_id}'::uuid, 'cancelled');` : ''}</pre>
        </details>
...
function RunFiles({ runId, tenant, live, onClose }: { runId: string; tenant: string; live: boolean; onClose: () => void }) {
  ...
  const filesRead = useReadResource({
    key: JSON.stringify(['ingest-files', tenant, runId, limit]),
    enabled: valid,
    refreshMs: live ? REFRESH_MS : 0,
    read: async (signal) => {
      ...
      return captureRows(
        result.rows,
        `Requested up to ${limit} file receipts for run ${runId}; active and failed files are returned first${result.truncated_at != null ? `; transport truncated at ${result.truncated_at}` : ''}.`,
        { operation: 'ops.ingest_files', run_id: runId, requested_limit: limit },
      );
    },
  });
  ...
  const columns: ResultColumn<IngestFile>[] = [
    ...
    { key: 'entities', label: 'Staged entities' },
    { key: 'physicalities', label: 'Staged physicalities' },
    { key: 'attestations', label: 'Staged attestations' },
    { key: 'bytes', label: 'Bytes' },
    { key: 'error', label: 'Error' },
  ];

  return (
    <Panel title="File receipts" expandable actions={<Button variant="ghost" onClick={onClose}>Close file receipts</Button>}>
      <code>{runId}</code>
      <div className={styles.toolbar}>
        <label className={styles.limitLabel}>
          Files
          <select className={styles.limitSelect} value={limit} onChange={(event) => setLimit(Number(event.target.value))}>
            {[250, 500, 1000, 5000].map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <Button variant="ghost" disabled={!valid} onClick={() => void filesRead.refresh()}>Refresh</Button>
      </div>
      {!valid
        ? <ErrorText>The run address is not a UUID. Choose a run from the journal.</ErrorText>
        : <ReadStatus label="File receipts" resource={filesRead} />}
      {filesRead.data && (
        <ResultWorkspace
          scopeKey={JSON.stringify(['ingest-files', tenant, runId])}
          label="Ingested file receipts"
          snapshot={filesRead.data}
          columns={columns}
          rowLabel={(file) => file.file_label}
          selectable={false}
          compact
        />
      )}
