...
BEGIN
    ...
    SELECT g.event_id INTO got FROM chess_player_games(tal, 25, 0) g LIMIT 1;
    ...
    SELECT as_white, outcome, opponent_id INTO b, l, got
      FROM chess_player_games(tal, 25, 0) WHERE event_id = e2;
    ...
    SELECT outcome INTO l FROM chess_player_games(tal, 25, 0) WHERE event_id = e4;
    ...
    SELECT g.eco INTO txt FROM chess_player_games(tal, 25, 0) g WHERE g.event_id = e1;
    ...
    SELECT opponent INTO txt FROM chess_player_games(tal, 25, 0) WHERE event_id = e1;
    ...
    SELECT count(*) INTO n FROM chess_game_plies(pl_event);
    IF n <> 3 THEN
    ...
    SELECT position_id INTO got FROM chess_game_plies(pl_event) WHERE ply = 1;
    IF got <> pl_p0 THEN RAISE EXCEPTION 'FAIL: vertex 1 is not the start position'; END IF;
    SELECT count(*) INTO n FROM chess_game_plies(pl_event) WHERE san IS NOT NULL;
    IF n <> 2 THEN
    ...
    SELECT count(*) INTO n FROM chess_game_plies(pl_event) WHERE move_quality IS NOT NULL;
    IF n <> 1 THEN
    ...
    SELECT count(*) INTO n FROM chess_game_plies(e1);
    IF n <> 0 THEN
        RAISE EXCEPTION 'FAIL: a line with no trajectory returned % rows instead of abstaining', n;
    END IF;
    SELECT count(*) INTO n FROM chess_game_plies(laplace_hash128_blake3('test/chess2/no_such_game'));
    IF n <> 0 THEN
