...
The normal writer does more than store a geometry. `NpgsqlWorkingSetApply.cs` owns native COPY-blob collection, managed tuple parsing and deduplication, presence verification, write epochs, replay journals, staged COPY, and the evidence/consensus transaction boundary. In the canonical identity generation, the existing apply advisory lock orders canonical entity selection and interpretation publication. Concurrent conflicts retry the complete apply operation and invalidate cached absence claims while retaining known positive identities. Its `WS_APPLY` lines report preparation, verification and COPY phases. `ApplyResult.RoundTrips` is a legacy logical phase counter; it is not a complete count of physical PostgreSQL protocol crossings. Actual COPY-transaction counters and commit acknowledgement receipts should be used when present.

The existing physicality throughput gate in `WriterThroughputTests.cs` stages 500,000 distinct ordered compositions of three genuine Unicode floor atoms. Each Content form carries its exact coordinate, Hilbert index, and three-vertex trajectory. Its timer includes normal physicality descriptor admission and the apply, while excluding input composition, the preceding entity seed, and the subsequent `CompleteBulkRunAsync` GIN drain and selected-row verification. The reported numerator is source forms, separate from the larger generated entity/physicality graph. Its 500,000-forms-per-second threshold is a test requirement, not an observed rate or a claim about complete recorded games.
