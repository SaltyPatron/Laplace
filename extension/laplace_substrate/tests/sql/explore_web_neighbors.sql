...
BEGIN
    ...
    SELECT count(*), bool_and(e.outbound)
      INTO self_rows, self_all_out
    FROM consensus.explore_web_neighbors(s1, 32) e
    WHERE e.nbr = s1 AND e.type_id = rel_a;
    IF self_rows <> 1 OR self_all_out IS DISTINCT FROM true THEN
        RAISE EXCEPTION 'FAIL: self edge rows=%, all_outbound=%', self_rows, self_all_out;
    END IF;
    ...
    FROM (
        ...
        HAVING count(e.nbr) <> 2
    ) q;
    IF wrong_roots <> 0 THEN
        RAISE EXCEPTION 'FAIL: % frontier roots did not receive exactly two slots', wrong_roots;
    END IF;
    ...
    RAISE NOTICE '✓ explore_web_neighbors: scalar/batch/default-partition parity, exact per-root limits, self-edge deduplication, and total tie order all hold';
END $$;
